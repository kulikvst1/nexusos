...
public:
    ...
    {
        // 1) Убираем окно из Desktop-пула (удаляем peer из Desktop::peers)
        removeFromDesktop();

        // 2) Отключаем меню, чтобы ни один MenuBarModel-слушатель не остался
        setMenuBar(nullptr, 0);

        // 3) Сохраняем состояние окна и всё остальное
        saveWindowState();
    }
