    ...
    : deviceMgr(getDefaultAudioDeviceManager())
{
    ...
    pluginScanPool = std::make_unique<ThreadPool>(jmax(1, SystemStats::getNumCpus() - 1));
    ...
    appProps = std::make_unique<ApplicationProperties>();
...
VSTHostComponent::~VSTHostComponent() noexcept
{
    ...
    // 4) снимаем параметр-слушателей и выгружаем плагин
    if (pluginInstance)
    {
    ...
    // 5) отключаем аудио и сбрасываем callback
    ...
    // 6) сбрасываем пул потоков
    pluginScanPool.reset();
...
{
    lastCpuLoad = load;
    // при желании: globalCpuLoad.store (load);
}
//==============================================================================
// setBpmDisplayLabel
...
void VSTHostComponent::setExternalPresetIndex(int idx) noexcept
{
    ...
    // опционально — уведомить внутренние колбэки:
    if (presetCb)
        presetCb(idx);
}
...
void VSTHostComponent::clearPluginScanJobs() noexcept
{
    pluginScanPool->removeAllJobs(-1, false); // или .removeAllJobs(false) в зависимости от версии JUCE
}

void VSTHostComponent::restartPluginScanPool()
{
    pluginScanPool.reset();
    pluginScanPool = std::make_unique<juce::ThreadPool>(std::max(1, juce::SystemStats::getNumCpus() - 1));
}

void VSTHostComponent::addPluginScanJob(juce::ThreadPoolJob* job, bool highPriority)
{
    pluginScanPool->addJob(job, highPriority);
}
int VSTHostComponent::getPluginScanJobCount() const noexcept
{
    return pluginScanPool
        ? pluginScanPool->getNumJobs()
        : 0;
}
//==============================================================================
// scanForPlugins
void VSTHostComponent::scanForPlugins()
{
    ...
    {
        auto cacheFile = settings->getFile().getSiblingFile(kPluginCacheFile);
    ...
    // 2) сканируем, если кеш пуст
    if (pluginList.getNumTypes() == 0)
    {
        using SL = File::SpecialLocationType;
        Array<File> dirs = {
            File::getSpecialLocation(SL::globalApplicationsDirectory)
                 .getChildFile("Common Files").getChildFile("VST3"),
            File::getSpecialLocation(SL::userDocumentsDirectory)
                 .getChildFile("VST3"),
            File::getSpecialLocation(SL::userHomeDirectory)
                 .getChildFile("VST3")
        };
        ...
        for (auto& dir : dirs)
        {
            ...
            dir.findChildFiles(vstFiles, File::findFiles, true, "*.vst3");

            for (auto& f : vstFiles)
            {
                // Заводим временный контейнер для только что найденных типов
                juce::OwnedArray<juce::PluginDescription> tmpDescriptions;
                pluginList.scanAndAddFile(f.getFullPathName(),
                    true,               // dontRescanExisting
                    tmpDescriptions,    // ← передаём именно контейнер
                    vst3);
                // (pluginList сам добавит новые типы в свой внутренний список)
            }
        }
    ...
    // 4) переносим в vector
    pluginFiles.clear();
    for (auto& desc : pluginList.getTypes())
        pluginFiles.push_back(File(desc.fileOrIdentifier));
}
