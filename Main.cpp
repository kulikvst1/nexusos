...
public:
    ...
    // Сеттер для установки указателя на AudioDeviceManager
    void setAudioDeviceManager(juce::AudioDeviceManager* dm)
    {
    ...
    MultiMidiSenderComponent()
    {
        ...
        // --- Добавляем CPU индикатор (компонент из cpu_load.h) ---/////////////////////////////////////////////////////////////////////////////////////
            ...
            {
                {
                    ...
                    // Уменьшаем высоту верхней панели - например, задаём 30 вместо 40
                    int topBarHeight = 30;
                    auto topBar = totalBounds.removeFromTop(topBarHeight);

                    // Выделяем нужную область для CPU-индикатора в правой части этой панели
                    int cpuIndicatorWidth = 120;
                    cpuIndicator->setBounds(topBar.removeFromRight(cpuIndicatorWidth).reduced(5));
    ...
    {
        // Если указатель определён, обновляем переменную загрузки
        if (audioDeviceManager && audioDeviceManager->getCurrentAudioDevice() != nullptr)
            globalCpuLoad.store(audioDeviceManager->getCpuUsage());
        else
            globalCpuLoad.store(0.0);
    }
...
private:
    ...
    //std::unique_ptr<VSTHostComponent> vstHostTab;
