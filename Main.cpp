...
public:
    ...
    // Создаем 3 вкладки: первая - пустая, вторая - основное окно с вашим UI, третья - пустая (на будущее)
        ...
        tabbedComponent->addTab("VST HOST", juce::LookAndFeel::getDefaultLookAndFeel().findColour(juce::ResizableWindow::backgroundColourId), vstHostComponent.get(), false);

        // Если нужно, сохраните его в качестве члена класса, чтобы управлять временем жизни:
        this->vstHostComponent = std::move(vstHostComponent);
...
private:
    ...
    std::unique_ptr<juce::Component> mainTab, vstHostComponent;
...
// Новый главный компонент, объединяющий UI и аудио DSP
class MainComponent : public juce::AudioAppComponent
{
public:
    MainComponent()
    {
        // Создаем ваш уже существующий UI-компонент (MultiMidiSenderComponent)
        // (Поскольку MultiMidiSenderComponent уже определён в Main.cpp, он просто создаётся здесь)
        multiMidiComp = std::make_unique<MultiMidiSenderComponent>();
        addAndMakeVisible(multiMidiComp.get());

        // Создаем объект аудио обработки (AudioDSP должен быть реализован в audio_dsp.h)
        audioDSP = std::make_unique<AudioDSP>();

        // Инициализируем аудиоустройство: 2 входа и 2 выхода
    ...
    // AudioAppComponent методы:
    void prepareToPlay(int samplesPerBlockExpected, double sampleRate) override
    {
        // Базовая подготовка аудио DSP (если требуется)
        audioDSP->prepareToPlay(samplesPerBlockExpected, sampleRate);

        // Если плагин уже загружен во вкладке VST HOST, подключаем его к аудиографу
        connectLoadedPlugin();
    }

    void getNextAudioBlock(const juce::AudioSourceChannelInfo& bufferToFill) override
    {
        audioDSP->getNextAudioBlock(bufferToFill);
    }

    void releaseResources() override
    {
        audioDSP->releaseResources();
    }

    void resized() override
    {
        // Располагаем UI-компонент на всю область MainComponent
        multiMidiComp->setBounds(getLocalBounds());
    }

    // Этот метод получает загруженный VST-плагин из MultiMidiSenderComponent 
    // (через аксессор getVSTHostComponent() – который вы должны добавить в MultiMidiSenderComponent)
    // и передает его в аудио DSP через метод prepareToPlayDSP.
    void connectLoadedPlugin()
    {
        if (multiMidiComp != nullptr)
        {
            // Получаем VSTHostComponent из вашего MultiMidiSenderComponent
            if (auto* vstHost = multiMidiComp->getVSTHostComponent())
            {
                // Получаем загруженный плагин. В классе VSTHostComponent должен быть метод:
                // juce::AudioPluginInstance* getPluginInstance() const { return pluginInstance.get(); }
                juce::AudioPluginInstance* pluginInst = vstHost->getPluginInstance();
                if (pluginInst != nullptr)
                {
                    // Передаем плагин в аудио обработку.
                    // Здесь для примера заданы фиксированные значения: samplesPerBlock = 512 и sampleRate = 44100.0
                    // Вы можете сохранить их при вызове prepareToPlay, если необходимо.
                    audioDSP->prepareToPlayDSP(512, 44100.0, pluginInst);
                }
            }
        }
    }
...
private:
    std::unique_ptr<MultiMidiSenderComponent> multiMidiComp;
    std::unique_ptr<AudioDSP> audioDSP;
...
//==============================================================================
// MainWindow — главное окно вашего приложения с поддержкой kiosk mode.
class MainWindow : public juce::DocumentWindow
{
public:
    // Здесь в конструкторе принимаются имя окна, указатель на MultiMidiSenderComponent и ссылка на AudioDeviceManager.
        ...
        deviceManager(adm), kioskModeEnabled(false)
    {
        ...
        // Вместо прямой установки mcomp, создаем MainComponent, который объединяет UI и аудио DSP.
        ...
        // Сохранение параметров kiosk mode...
        {
            juce::PropertiesFile::Options options;
            options.applicationName = "MIDI Preset Scenes";
            juce::File configFile = juce::File::getSpecialLocation(juce::File::userApplicationDataDirectory)
    ...
    void closeButtonPressed() override
    {
        if (auto* comp = dynamic_cast<MultiMidiSenderComponent*>(mainComponent->getContentComponent()))
            comp->saveSettings();
        juce::JUCEApplication::getInstance()->systemRequestedQuit();
    }
...
private:
    juce::AudioDeviceManager& deviceManager;
    MainComponent* mainComponent; // Теперь содержимое окна – наш MainComponent
