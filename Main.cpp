...
{
public:
    // При создании MainComponent передается указатель на MultiMidiSenderComponent и ссылка на общий AudioDeviceManager adm
        ...
        : multiMidiComp(mcomp), deviceManager(adm)
    {
        ...
        // Используем общий AudioDeviceManager для AudioDSP
        audioDSP = std::make_unique<AudioDSP>(deviceManager);

        // Настраиваем каналы — это уже будет общая настройка
    ...
    {
        // Передаем настройки в AudioDSP (без вызова setAudioChannels внутри AudioDSP)
        audioDSP->prepareToPlayDSP(samplesPerBlockExpected, sampleRate, getLoadedPluginInstance());
        // Например, функция connectLoadedPlugin() тоже может использовать общий объект
        connectLoadedPlugin();
    }
    ...
    // Примерная функция для получения загруженного плагина (реализуйте по необходимости)
    juce::AudioPluginInstance* getLoadedPluginInstance()
    {
        if (multiMidiComp != nullptr)
        {
            return multiMidiComp->getVSTHostComponent() != nullptr ?
                multiMidiComp->getVSTHostComponent()->getPluginInstance() : nullptr;
        }
        return nullptr;
    }
    ...
    {
        // Если требуется переинициализировать цепочку обработки плагина,
        // здесь можно выполнить дополнительную логику подключения
    }
