...
public:
    ...
    void buttonClicked(juce::Button* button) override
    {
        ...
        // Обработка стандартных CC‑кнопок (на основе вашего кода)
        for (int i = 0; i < ccButtons.size(); ++i)
        {
            if (ccButtons[i] == button)
            {
                ...
                // Прямое управление параметрами плагина.
                if (vstHostComponent != nullptr && vstHostComponent->getPluginInstance() != nullptr)
                {
                    vstHostComponent->setPluginParameterFromCC(i, ccValue);
                }

                // Обработка пресетной информации – обновляем состояние в выбранном банке
    ...
    void updatePresetButtonLabels()
    {
        if (auto* bm = dynamic_cast<BankManager*>(bankTab.get()))
        {
            ...
            if (activeIndex >= 0 && activeIndex < bm->getBanks().size())
            {
                // Определяем индекс выбранного пресета с учетом состояния SHIFT
                int groupOffset = shiftButton->getToggleState() ? 3 : 0;
                int selectedPreset = -1;
                for (int j = 0; j < presetButtons.size(); ++j)
                {
                ...
                // Получаем сохраненное состояние для CC-кнопок выбранного пресета
                const auto& ccStates = bm->getBanks()[activeIndex].ccPresetStates[presetIndex];

                for (int i = 0; i < ccButtons.size(); ++i)
                {
                    // Если для данной кнопки сохранено состояние, используем его; иначе false
                    bool state = (i < ccStates.size()) ? ccStates[i] : false;
                    ccButtons[i]->setToggleState(state, juce::dontSendNotification);
                    int ccValue = state ? 64 : 0;
                    int ccNumber = i + 1;

                    // Отправляем MIDI-сообщение (если требуется)
                    if (midiOut != nullptr)
                    {
                        auto msg = juce::MidiMessage::controllerEvent(1, ccNumber, (juce::uint8)ccValue);
                        handleOutgoingMidiMessage(msg);
                    ...
                    // И сразу обновляем параметры плагина напрямую
                    if (vstHostComponent != nullptr && vstHostComponent->getPluginInstance() != nullptr)
                    {
                        vstHostComponent->setPluginParameterFromCC(i, ccValue);
                    }
        ...
        // Здесь можно обновить и другие элементы (например, volumeSlider, bankNameLabel и т.д.)
    }
