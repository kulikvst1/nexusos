...
public:
    ...
    {
        // Разбиваем окно на 9 столбцов и 4 строки (36 секторов)
        int numCols = 9;
        int numRows = 4;
        auto bounds = getLocalBounds().reduced(10);
        int sectorWidth = bounds.getWidth() / numCols;
        int sectorHeight = bounds.getHeight() / numRows;

        auto getSectorRect = [=](int sectorNumber) -> juce::Rectangle<int>
            {
                int idx = sectorNumber - 1; // 0-базовая нумерация
                int row = idx / numCols;
                int col = idx % numCols;
                return juce::Rectangle<int>(col * sectorWidth, row * sectorHeight, sectorWidth, sectorHeight);
            };
        ...
        // Размещение по секторам:
        // Сектор 1: Gain-слайдер и label. Label центрируется по верхней части сектора,
        // слайдер занимает центральную часть.
        {
            auto r = getSectorRect(1).reduced(4);
        ...
        {
            auto r = getSectorRect(9).reduced(4);
            auto labelArea = r.removeFromTop(20);
        ...
        // Сектор 27: кнопка SHIFT (по вашим новым требованиям, shift должен быть в секторе 27).
        if (shiftButton)
        ...
        // Сектора 11-17: группа стандартных CC-кнопок. Объединяем их в один прямоугольник и располагаем горизонтально.
        {
