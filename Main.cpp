...
{
public:
    // При создании MainComponent передаются указатель на MultiMidiSenderComponent и ссылка на общий AudioDeviceManager.
    MainComponent::MainComponent(MultiMidiSenderComponent* mcomp, juce::AudioDeviceManager& adm)
    ...
    {
        // Вызывается метод AudioAppComponent для остановки аудио-цикла и удаления аудио callback
        shutdownAudio();
    }

    // Метод prepareToPlay вызывается перед началом воспроизведения аудио.
    // Здесь мы также передаём актуальные sample rate и block size в загруженный плагин.
    void MainComponent::prepareToPlay(int samplesPerBlockExpected, double sampleRate)
{
    if (auto* vstHost = multiMidiComp->getVSTHostComponent())
    {
        ...
        DBG("MainComponent: plugi plugin.");
    }
    if (auto* plugin = getLoadedPluginInstance())
    {
        plugin->prepareToPlay(sampleRate, samplesPerBlockExpected);
        DBG("MainComponent: Plugin prepared with sampleRate = " << sampleRate << ", block size = " << samplesPerBlockExpected);
    }
    else
    {
        DBG("MainComponent: No plugin loaded in prepareToPlay.");
    }
    connectLoadedPlugin();
}
    ...
    // Здесь получаем аудиобуфер, передаём его в плагин для обработки и очищаем неактивные каналы.
    void getNextAudioBlock(const juce::AudioSourceChannelInfo& bufferToFill) override
    {
        if (auto* plugin = getLoadedPluginInstance())
        {
            ...
            DBG("MainComponent: processBlock executed on plugin.");

            // Используем наш AudioDeviceManager для получения текущего аудиоустройства.
            if (auto* currentDevice = deviceManager.getCurrentAudioDevice())
            {
                auto activeChannels = currentDevice->getActiveOutputChannels();
                for (int channel = 0; channel < bufferToFill.buffer->getNumChannels(); ++channel)
                {
                    // Если для конкретного канала активность равна false, очищаем его буфер.
                    if (!activeChannels[channel])
                    {
                        bufferToFill.buffer->clear(channel, bufferToFill.startSample, bufferToFill.numSamples);
                        DBG("MainComponent: Cleared inactive output channel " << channel);
                    }
                }
            }
        }
        ...
        {
            // Если плагин не загружен, полностью очищаем буфер.
           // DBG("no plugin load clean bufer");
            bufferToFill.clearActiveBufferRegion();
        }
    }

    // Метод, вызываемый при освобождении ресурсов аудио-системы.
    ...
    // Изменение размеров компонента – изменяем размеры отображаемого компонента MultiMidiSenderComponent.
    void resized() override
    {
    ...
    juce::AudioPluginInstance* getLoadedPluginInstance()
    {
        ...
        {
            auto* vstHost = multiMidiComp->getVSTHostComponent();
            if (vstHost == nullptr)
            {
                DBG("getLoadedPluginInstance: getVSTHostComponent() returned nullptr");
                return nullptr;
            }
            else
            {
                auto* inst = vstHost->getPluginInstance();
                DBG("getLoadedPluginInstance: getPluginInstance() returned "
                    + juce::String::formatted("%p", inst));
                return inst;
            }
        }
        DBG("getLoadedPluginInstance: multiMidiComp is nullptr");
        return nullptr;
    }


    // Дополнительная логика для подключения плагина по необходимости.
    void connectLoadedPlugin()
    {
        // Здесь можно добавить, например, обновление цепочки сигналов или дополнительные вызовы настроек плагина.
    }

private:
    MultiMidiSenderComponent* multiMidiComp = nullptr;
    // Ссылка на внешний объект AudioDeviceManager, который используется для получения текущего устройства.
