...
public:
    ...
    {
        // --- Инициализация стандартных 10 кнопок для MIDI CC ---
        for (int i = 0; i < 10; ++i)
        {
        ...
        // --- Инициализация 3 кнопок-пресетов (например, для групп A–C) ---
        for (int i = 0; i < 3; ++i)
        {
        ...
        // --- Инициализация номеров MIDI CC по умолчанию ---
        shiftCCNumber = 90;
        upCCNumber = 72;
        downCCNumber = 73;
        tempoCCNumber = 74;

        // Для пресетов – 6 групп (например, A=80, B=81, ... F=85)
        ...
        gainSlider->setValue(64); // значение по умолчанию
        ...
        volumeSlider->setValue(64); // значение по умолчанию
        ...
        // --- Логика сохранения состояния Volume по пресетам ---
        // Сохраняем положение слайдера Volume для каждого из 6 пресетов (по умолчанию 64)
        presetVolume.resize(6, 64);
        currentPresetIndex = -1; // Пока ни один пресет не выбран

        // Загружаем сохранённые настройки (включая положение слайдеров Gain и Volume, а также пресетов)
        loadSettings();
    }
    ...
    // --- Отправка исходящего MIDI-сообщения (с задержкой в 1 мс) ---
    ...
    // --- Открытие выбранного MIDI-устройства для вывода ---
    void openMidiOut(const juce::MidiDeviceInfo& device)
    {
        DBG("Попытка открыть MIDI Output: " << device.name << ", id: " << device.identifier);
        if (midiOut != nullptr)
            midiOut.reset();
        ...
        if (midiOut != nullptr)
        {
            ...
            DBG("Открыт MIDI Output: " << device.name << " (ID = " << currentMidiOutputID << ")");
        }
        else
        {
            DBG("Не удалось открыть MIDI Output: " << device.name);
        }
    }

    // --- Обновление MIDI Output ---
    void updateMidiOutputDevice(const juce::MidiDeviceInfo& device)
    {
        openMidiOut(device);
    }
    // --- Обработка нажатия кнопок (Button::Listener) ---
    void buttonClicked(juce::Button* button) override
    {
        // Обработка кнопок TEMPO, UP и DOWN
        ...
        for (int i = 0; i < ccButtons.size(); ++i)
        {
            if (ccButtons[i] == button)
            {
                ...
                // Если выбран пресет – запоминаем состояние кнопки
        ...
        if (button->getRadioGroupId() == 100)
        {
            ...
            if (clickedIndex != -1 && button->getToggleState())
            {
                ...
                // При выборе пресета, устанавливаем значение слайдера Volume для данного пресета
                volumeSlider->setValue(presetVolume[presetIndex], juce::dontSendNotification);
    ...
    void sliderValueChanged(juce::Slider* slider) override
    {
        ...
        {
            int value = static_cast<int>(gainSlider->getValue());
            if (midiOut != nullptr)
            {
                auto msg = juce::MidiMessage::controllerEvent(1, gainCCNumber, (juce::uint8)value);
                handleOutgoingMidiMessage(msg);
            }
            // Здесь изменение Gain-слайдера просто сохраняется через его значение при сохранении настроек.
        }
        else if (slider == volumeSlider.get())
        {
            int value = static_cast<int>(volumeSlider->getValue());
            if (midiOut != nullptr)
            {
                auto msg = juce::MidiMessage::controllerEvent(1, volumeCCNumber, (juce::uint8)value);
                handleOutgoingMidiMessage(msg);
            }
            // Если активен пресет, обновляем сохраненное значение Volume для него
            if (currentPresetIndex != -1)
               presetVolume[currentPresetIndex] = value;
        }
    }

    // --- Обработка входящих MIDI-сообщений (MidiInputCallback) ---
    ...
    // --- Формирование подписи для кнопок-пресетов (учитывая состояние SHIFT) ---
    ...
    // Отправка миди кнопок сс
    void sendCCButtonStates()
    {
        for (int i = 0; i < ccButtons.size(); ++i)
        {
            // Получаем состояние кнопки: если "on", отправляем значение (например) 64, иначе – 0.
            bool state = ccButtons[i]->getToggleState();
            int ccValue = state ? 64 : 0;
            // Здесь можно использовать номер CC как (i + 1) или взять его из настроек
            int ccNumber = i + 1;
    ...
    // --- Раскладка (layout) элементов компонента ---

    void resized() override
    {
        // Определяем отступ для всех сторон (например, 10 пикселей)
        const int margin = 10;

        // Получаем рабочую область с отступами: отступы слева, справа, сверху и снизу = margin
        auto content = getLocalBounds().reduced(margin);

        // Разбиваем оставшуюся область на 9 столбцов и 4 строки (36 секторов)
        int numCols = 9;
        int numRows = 4;

        // Вычисляем базовую ширину столбца внутри рабочей области
        int usableWidth = content.getWidth();
        int sectorWidth = usableWidth / numCols;
        // Вычисляем лишние пиксели, которые не хватает до полного заполнения
        int extra = usableWidth - (sectorWidth * numCols);

        int sectorHeight = content.getHeight() / numRows;

        // Лямбда для вычисления прямоугольника одного сектора.
        // Здесь учитываем смещение рабочей области (content.getX() и content.getY()),
        // чтобы отступы слева и справа были равны margin.
        auto getSectorRect = [=](int sectorNumber) -> juce::Rectangle<int>
            {
                int idx = sectorNumber - 1; // перевод в 0-базированную нумерацию
                int row = idx / numCols;
                int col = idx % numCols;

                // Вычисляем X-координату для этого столбца, начиная с content.getX()
                int x = content.getX();
                for (int c = 0; c < col; ++c)
                    x += sectorWidth + (c < extra ? 1 : 0);

                // Ширина данного столбца = базовая ширина + 1 пиксель, если он входит в первые extra колонок
                int w = sectorWidth + (col < extra ? 1 : 0);

                // Y вычисляется с учетом начала рабочей области (content.getY())
                int y = content.getY() + row * sectorHeight;
                return juce::Rectangle<int>(x, y, w, sectorHeight);
            };

        // Лямбда для объединения нескольких секторов по горизонтали (например, для кнопок-пресетов)
        ...
        // Раскладка по секторам (остальные вызовы setBounds() остаются без изменений):
        {
            ///////////////////////////////// Сектор 1: Gain-слайдер и label.
            auto gainSector = getSectorRect(1).reduced(4);
            gainSlider->setBounds(gainSector);
            int labelWidth = gainSector.getWidth() / 2;
            int labelHeight = 20;
            juce::Rectangle<int> labelRect(labelWidth, labelHeight);
            ...
            ///////////////////////////////// Сектор 9: Volume-слайдер и label.
            auto sliderArea = getSectorRect(9).reduced(4);
            volumeSlider->setBounds(sliderArea);
            int volLabelWidth = sliderArea.getWidth() * 2;
            int volLabelHeight = 30;
            juce::Rectangle<int> volLabelRect(volLabelWidth, volLabelHeight);
            volLabelRect.setCentre(sliderArea.getCentre());
            volumeLabel.setBounds(volLabelRect);
            volumeLabel.setJustificationType(juce::Justification::centred);
            volumeLabel.setColour(juce::Label::outlineColourId, juce::Colours::transparentBlack);

            // Сектор 10: кнопка UP.
            if (upButton)
                upButton->setBounds(getSectorRect(10).reduced(4));

            // Сектор 18: кнопка TEMPO.
            if (tempoButton)
                tempoButton->setBounds(getSectorRect(18).reduced(4));

            // Сектор 19: кнопка DOWN.
            if (downButton)
                downButton->setBounds(getSectorRect(19).reduced(4));

            // Сектор 27: кнопка SHIFT.
            if (shiftButton)
                shiftButton->setBounds(getSectorRect(27).reduced(4));

            // Сектора 28-30: кнопка-пресет A (объединяем сектора 28..30)
            if (presetButtons.size() > 0)
            {
                auto r = getUnionRect(28, 30).reduced(4);
                presetButtons[0]->setBounds(r);
            }

            // Сектора 31-33: кнопка-пресет B.
            if (presetButtons.size() > 1)
            {
                auto r = getUnionRect(31, 33).reduced(4);
                presetButtons[1]->setBounds(r);
            }

            // Сектора 34-36: кнопка-пресет C.
            if (presetButtons.size() > 2)
            {
                auto r = getUnionRect(34, 36).reduced(4);
                presetButtons[2]->setBounds(r);
            }

            // Сектора 11-17: группа стандартных CC-кнопок.
            // Объединяем их в один прямоугольник, уменьшаем его высоту в 2 раза
            {
                auto r = getUnionRect(11, 17).reduced(4);
                // Уменьшаем высоту области в два раза:
                r = r.withHeight(r.getHeight() / 2);

                int numButtons = ccButtons.size();
                int buttonWidth = r.getWidth() / numButtons;
                for (int i = 0; i < numButtons; ++i)
                    ccButtons[i]->setBounds(r.removeFromLeft(buttonWidth).reduced(2));
            }
            // Текстовая метка BANK NAME в области, соответствующей секторам 2-8 (верхний ряд, центральная часть)
            {
                ...
                bankNameLabel.setColour(juce::Label::backgroundColourId, juce::Colours::blue);
    ...
    void loadSettings()
    {
        ...
        // Загрузка сохранённого значения rotary-слайдера Gain
    ...
    void saveSettings()
    {
        ...
        // Сохранение состояния Gain-sлайдера
        propertiesFile.setValue("gainSliderValue", gainSlider->getValue());
        // Сохранение состояния Volume для каждого пресета
        for (int i = 0; i < presetVolume.size(); i++)
        {
...
private:
    ...
    // --- Логика сохранения состояния Volume для пресетов ---
    // Хранится положение слайдера Volume для каждого из 6 пресетов.
    std::vector<int> presetVolume;
    // Индекс активного пресета; если ни один не выбран, currentPresetIndex = -1.
