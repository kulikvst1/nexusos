...
public:
    ...
    VSTHostComponent* getVSTHostComponent() { return vstHostTab.get(); }
...
public:
    ...
    // Обратите внимание: мы не вызываем здесь setAudioChannels(), чтобы не затирать настройки, установленные пользователем через меню.
        ...
        : multiMidiComp(mcomp), deviceManager(adm)
    {
        ...
        // Если необходимо задать значения по умолчанию для устройства, их можно установить через adm (например, используя AudioDeviceSelectorComponent),
        // а не через вызов setAudioChannels здесь.
        setSize(800, 600);
    }
    ...
    // Метод prepareToPlay вызывается перед началом воспроизведения.
    // Если плагин загружен, он получает актуальные sampleRate и blockSize.
    void prepareToPlay(int samplesPerBlockExpected, double sampleRate) override
    {
        if (auto* plugin = getLoadedPluginInstance())
        {
            plugin->prepareToPlay(sampleRate, samplesPerBlockExpected);
            DBG("MainComponent: Plugin prepared with sampleRate = "
                << sampleRate << ", block size = " << samplesPerBlockExpected);
        }
        else
        {
    ...
    // Основной аудиовызов.
    // Если плагин загружен – передаём ему буфер для обработки, затем очищаем неактивные каналы согласно настройкам устройства.
    void getNextAudioBlock(const juce::AudioSourceChannelInfo& bufferToFill) override
    {
        if (auto* plugin = getLoadedPluginInstance())
        {
            ...
            // Получаем текущее аудиоустройство, чтобы узнать активные выходные каналы.
            if (auto* currentDevice = deviceManager.getCurrentAudioDevice())
            {
                auto activeChannels = currentDevice->getActiveOutputChannels();
                for (int channel = 0; channel < bufferToFill.buffer->getNumChannels(); ++channel)
                {
                    // Если для конкретного канала активность равна false, очищаем его область.
    ...
    // Дополнительная логика для подключения плагина, если требуется.
    void connectLoadedPlugin()
    {
        // Здесь можно выполнить дополнительную логику (например, переинициализацию плагина или обновление цепочки сигналов).
    }
