    ...
    MultiMidiSenderComponent()
    {
        ...
        if (bm != nullptr)
        {
            bm->onBankManagerChanged = [this, bm]()
                {
                    // Получаем активный банк из менеджера банков:
                    auto activeBank = bm->getBank(bm->getActiveBankIndex());
                    // Обновляем отображение имени банка в главном окне:
                    bankNameLabel.setText(activeBank.bankName, juce::dontSendNotification);

                    // Обновляем подписи кнопок-пресетов в зависимости от состояния SHIFT:
                    if (shiftButton->getToggleState())
                    {
                        // Если кнопка SHIFT активна, отображаем имена пресетов №4, 5, 6 (индексы 3,4,5)
                        for (int i = 0; i < presetButtons.size(); ++i)
                        {
                            // Предполагается, что presetButtons содержит ровно 3 кнопки
                            if (i < 3)
                    ...
                    {
                        // Если SHIFT не активен, отображаем имена пресетов №1, 2, 3 (индексы 0,1,2)
                        for (int i = 0; i < presetButtons.size(); ++i)
                        {
                            if (i < 3)
