...
void BankEditor::setActivePreset(int newPreset)
{
    ...
    // 1) синхронизируем toggle-кнопки UI
    for (int i = 0; i < numCCParams; ++i)
        ccToggleButtons[i].setToggleState(
            banks[activeBankIndex].ccPresetStates[activePreset][i],
            juce::dontSendNotification);

    // 2) прокидываем 10 CC-сообщений
    for (int i = 0; i < numCCParams; ++i)
    {
    ...
    // 3) пушим уведомление и сохраняем
    sendChange();
    saveSettings();
}
...
{
    // 1) забираем слот
    auto& slot = banks[activeBankIndex].ccMappings[index];

    // 2) считаем значение (твоя формула из Main.cpp)
    uint8_t effective = (slot.enabled && slot.paramIndex >= 0)
        ? (state ? slot.ccValue : 0)
        : (state ? 127 : 0);
    if (slot.invert)
        effective = 127 - effective;

    // 3) шлём MIDI (канал = 1, контроллер = index+1)
    ...
    // 4) шлём плагину
    if (vstHost != nullptr && slot.enabled && slot.paramIndex >= 0)
        vstHost->setPluginParameter(slot.paramIndex, effective);

    // (не сохраняем здесь, сохраним в вызывающем коде)
}
