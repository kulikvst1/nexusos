...
{
public:
    explicit LooperComponent(LooperEngine& eng, bool embeddedInRig = false);
    ...
    juce::String getControlButtonText() const;
    juce::Colour getControlButtonColor() const;
    void setRigControl(Rig_control* rig);

    void pressResetButton() { resetBtn.triggerClick(); }
    void pressControlButton() { controlBtn.triggerClick(); }
    void handleReset();
    void handleControl();
    void setLevelFromRig(float db);
    void setLevelMidi(int value);
    float getLevelSliderValue() const noexcept;
...
private:
   ...
   // SliderTextBoxLNF lnf;//digitalkey
