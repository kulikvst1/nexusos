...
public:
    ...
    BankEditor(PluginManager& pm, VSTHostComponent* host = nullptr, bool loadDefaultFlag = false);
  ...
  //
    ...
    //
    std::function<void(const juce::File&)> onLibraryFileChanged;
    juce::String getCurrentPluginDisplayName() const;
    std::function<void()> onPluginChanged;
    std::function<void(int slot, bool state)> onCCStateChanged;
    void addCCStateListener(std::function<void(int, bool)> listener)
    {
        ccListeners.push_back(std::move(listener));
    }

    void notifyCCStateChanged(int slot, bool active);
    const juce::File& getCurrentlyLoadedBankFile() const noexcept { return currentBankFile; }
...
private:
   ...
   juce::TextButton configSelectButton;
   bool shouldLoadDefaultOnStartup = false;
   std::vector<std::function<void(int, bool)>> ccListeners;
