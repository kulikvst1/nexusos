...
{
    DBG("→ updateCoefficients: lowCut=" << lowCutFreq
        << " lowGain=" << lowGain
        << " midGain=" << midGain
        << " highGain=" << highGain
        << " highCut=" << highCutFreq);
    ...
    // Генерируем новые коэффициенты
    auto lowCutCoefs = juce::IIRCoefficients::makeHighPass(currentSampleRate, lowCutFreq, defaultQ);
    auto lowShelfCoefs = juce::IIRCoefficients::makeLowShelf(currentSampleRate, lowFreq, defaultQ, lowGain);
    auto peakCoefs = juce::IIRCoefficients::makePeakFilter(currentSampleRate, midFreq, midQ, midGain);
    auto highShelfCoefs = juce::IIRCoefficients::makeHighShelf(currentSampleRate, highFreq, defaultQ, highGain);
    auto highCutCoefs = juce::IIRCoefficients::makeLowPass(currentSampleRate, highCutFreq, defaultQ);

    // Применяем их ко всем каналам
