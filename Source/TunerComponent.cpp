...
{ 
    ...
    rmsSlider.setSliderStyle(juce::Slider::RotaryHorizontalVerticalDrag);
    rmsSlider.setTextBoxStyle(juce::Slider::TextBoxBelow, false, 60, 20);
    rmsSlider.setRange(0.001, 0.02, 0.001); // диапазон RMS порогов
    rmsSlider.setValue(rmsOn);              // начальное значение
    rmsSlider.onValueChange = [this] {
        rmsOn = (float)rmsSlider.getValue();
        rmsOff = rmsOn * 0.5f;              // off всегда ниже on
        };

    // Confidence slider
    addAndMakeVisible(confSlider);
    confSlider.setSliderStyle(juce::Slider::RotaryHorizontalVerticalDrag);
    confSlider.setTextBoxStyle(juce::Slider::TextBoxBelow, false, 60, 20);
    confSlider.setRange(0.5, 0.95, 0.01);   // диапазон доверия
    confSlider.setValue(0.90);              // начальное значение
    confSlider.onValueChange = [this] {
        confOn = (float)confSlider.getValue();
        confOff = confOn - 0.25f;           // off всегда ниже on
        };

    rmsSlider.setColour(juce::Slider::textBoxTextColourId, juce::Colours::black);
    rmsSlider.setColour(juce::Slider::textBoxBackgroundColourId, juce::Colours::white);
    rmsSlider.setColour(juce::Slider::rotarySliderFillColourId, juce::Colours::red);
    confSlider.setColour(juce::Slider::rotarySliderFillColourId, juce::Colours::red);
    confSlider.setColour(juce::Slider::textBoxTextColourId, juce::Colours::black);
    confSlider.setColour(juce::Slider::textBoxBackgroundColourId, juce::Colours::white);

    // RMS label
    addAndMakeVisible(rmsLabel);
    rmsLabel.setText("Threshold", juce::dontSendNotification);
    rmsLabel.setColour(juce::Label::backgroundColourId, juce::Colours::white);
    rmsLabel.setColour(juce::Label::textColourId, juce::Colours::black);
    rmsLabel.setJustificationType(juce::Justification::centred);

    // Confidence label
    addAndMakeVisible(confLabel);
    confLabel.setText("Filter", juce::dontSendNotification);
    confLabel.setColour(juce::Label::backgroundColourId, juce::Colours::white);
    confLabel.setColour(juce::Label::textColourId, juce::Colours::black);
...
void TunerComponent::resized()
{
    ...
    // Центрирование по горизонтали (кнопки A4)
    ...
    // ─────────────── Новые слайдеры ───────────────
    const int knobSize = juce::jmin(w / 6, h / 4);
    const int knobYOffset = 40;
    const int knobY = 10 + knobYOffset;

    // Левый верхний угол (RMS sensitivity)
    rmsSlider.setBounds(10, knobY, knobSize, knobSize);
    rmsLabel.setBounds(10, knobY + knobSize + 8, knobSize, knobSize / 4);

    // Правый верхний угол (Confidence sensitivity)
    confSlider.setBounds(w - knobSize - 10, knobY, knobSize, knobSize);
    confLabel.setBounds(w - knobSize - 10, knobY + knobSize + 8, knobSize, knobSize / 4);

    // Крупный шрифт для меток
    const int labelFontSize2 = juce::jmax(10, (int)(controlH * 0.3f));
    rmsLabel.setFont(juce::Font((float)labelFontSize2, juce::Font::bold));
    confLabel.setFont(juce::Font((float)labelFontSize2, juce::Font::bold));
