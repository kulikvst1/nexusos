    ...
    const juce::AudioIODeviceCallbackContext&)
{
    ...
    //    — пропускаем, если looper==nullptr или ещё не prepared()
    if (looper != nullptr && looper->isPreparedSuccessfully())
        looper->process(wetBuffer);
    // иначе — wetBuffer остаётся чистым копированием dryBuffer,
    // и вы просто получите «сухой» звук без лупа, но без краша!
    ...
    for (int ch = 0; ch < numOutputChannels; ++ch)
    {
        ...
        // --- СТАРАЯ ЛОГИКА (закомментирована): всегда dry + wet ---
        /*
        // сначала dry (плагин)
        ...
        */
        ...
        // --- НОВАЯ ЛОГИКА: dry всегда, wet только в режиме Playing ---
        // 1) копируем сухой сигнал плагина
        juce::FloatVectorOperations::copy(out,
            dryBuffer.getReadPointer(dryCh),
            numSamples);

        // 2) если лупер воспроизводит — прибавляем wet (петлю)
        if (looper != nullptr
        ...
        // иначе — остаётся только dry без удвоения громкости
    }
