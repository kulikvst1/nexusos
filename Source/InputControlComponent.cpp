...
InputControlComponent::InputControlComponent()
{
    ...
    // Калибровка входных индикаторов: смещаем порог на +3 dB
   // vuLeft.setCalibrationDb(6.0f);
   // vuRight.setCalibrationDb(6.0f);
...
void InputControlComponent::processBlock(float* const* channels, int numSamples) noexcept
{
    ...
    if (auto* leftCh = channels[0])
    {
        ...
        else
        {
            ...
            // проверка клипа в dB
            auto lvlDb = juce::Decibels::gainToDecibels(peak, -60.0f);
            if (lvlDb >= 0.0f)
            {
    ...
    // Захватываем всё нужное по значению ДО постинга
    ...
    juce::MessageManager::callAsync([self, preEnvL, preEnvR, clipLVal, clipRVal]()
        {
            ...
            // обновляем LevelMeterComponent
            self->vuLeft.setLevel(preEnvL);
            self->vuRight.setLevel(preEnvR);

            // обновляем атомарные переменные для геттеров
            self->vuLeftLevel.store(preEnvL, std::memory_order_relaxed);
            self->vuRightLevel.store(preEnvR, std::memory_order_relaxed);

            // обновляем clip-индикаторы
            self->clipLabelL.setVisible(clipLVal);
            self->clipLabelR.setVisible(clipRVal);
        });

    if (onInputClipChanged)
        onInputClipChanged(clipLVal, clipRVal);
}
...
//++++++++++++++++++++++++++++ FOOT PEDAL CONTROL ++++++++++++++++++++++++++++++++++
void InputControlComponent::syncSwitchState(int switchIndex, bool isOn)
{
    if (switchIndex == 1)
    {
...
void InputControlComponent::syncPedalSliderByCC(int cc, int value)
{
    ...
    case 5: // Pedal Min
        ...
        if (rigControl) rigControl->sendPedalMin(value);
        break;

    case 6: // Pedal Max
        pedalMaxSlider.setValue(value, juce::dontSendNotification);
        if (rigControl) rigControl->sendPedalMax(value);
        break;
    ...
    case 9: // Pedal Threshold
        ...
        if (rigControl) rigControl->sendPedalThreshold(value);
        break;
...
void InputControlComponent::syncAutoConfigButton(bool isOn)
{
    ...
    if (rigControl)
        rigControl->sendPedalAutoConfig(isOn);
}
