...
void BankEditor::editCCParameter(int ccIndex)
{
    ...
    new SetCCDialog(vstHost, initialMap, slotName,
        [this, ccIndex](CCMapping newMap, bool ok)
        {
            ...
            checkForChanges();
            // 🔑 Синхронизация с VSTHost
            if (vstHost != nullptr)
                vstHost->updateCCLabel(ccIndex, global.name);
...
void BankEditor::resetCCSlotState(int slot)
{
    ...
    // 🔑 Добавляем синхронизацию с VSTHost
    if (vstHost != nullptr)
        vstHost->updateCCLabel(slot, "<none>");
}
...
void BankEditor::onPluginParameterChanged(int paramIdx, float normalised)
{
    ...
    notifyCCStateChanged(slot, shouldBeEnabled);
...
void BankEditor::notifyCCStateChanged(int slot, bool active)
{
    // 🔹 сначала вызываем старый одиночный колбек (если кто-то ещё использует)
    if (onCCStateChanged)
        onCCStateChanged(slot, active);

    // 🔹 затем уведомляем всех слушателей
    for (auto& l : ccListeners)
        l(slot, active);
}
