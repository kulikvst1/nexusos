    ...
    bankNameEditor.onTextChange = [this]()
        {
            ...
            bankIndexLabel.setText("Bank: " + juce::String(activeBankIndex),
...
void BankEditor::resized()
{
    ...
    // Font & alignment for 0-row labels
    auto bigFont = juce::Font(sH * 0.7f, juce::Font::bold);
    bankIndexLabel.setFont(bigFont);
    bankIndexLabel.setJustificationType(juce::Justification::centred);
    bankIndexLabel.setBounds(r.removeFromTop(24));

    pluginLabel.setFont(bigFont);
    pluginLabel.setJustificationType(juce::Justification::centred);
    pluginLabel.setBounds(r.removeFromTop(20));
    // Row 0
    ...
    // Row 2: bankNameEditor, presetEditors, pluginLabel
    int fw = W / 8, fy = 2 * sH;
    ...
    // Row 6: ccToggleButtons
    for (int i = 0; i < numCCParams; ++i)
        ccToggleButtons[i].setBounds(i * 2 * sW, 6 * sH, 2 * sW, sH);

    // Row 11: bottom action buttons
    ...
    if (vstHost->getPluginInstance() == nullptr)
    {
        ...
        menu.showMenuAsync(
            ...
            [this](int choice)
            {
                ...
                pluginLabel.setText("Plugin: " + fileToLoad.getFileName(),
                    juce::dontSendNotification);
