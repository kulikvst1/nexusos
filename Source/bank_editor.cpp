...
void BankEditor::onPluginParameterChanged(int paramIdx, float normalised)
{
    ...
    if (vstHost == nullptr || vstHost->getPluginInstance() == nullptr)
        return;           // если плагина больше нет – игнорируем коллбэк
    ///////////////////////////////////////////////////////////////////////////////////////////////////////////
    if (isSettingPreset) return;

    const int v127 = juce::jlimit(0, 127,
        int(std::round(normalised * 127.f)));

    /* 1. Находим CC-слот, привязанный к этому параметру */
    int slot = -1;
    for (int i = 0; i < numCCParams; ++i)
    ...
    const bool invert = preset.invert;   // пресетная инверсия

    /* 2. Определяем, должна ли кнопка быть ON */
    const bool shouldBeEnabled = invert ? (v127 == 0) : (v127 > 0);

    /* 3. Переключаем кнопку, если надо */
    if (preset.enabled != shouldBeEnabled)
    {
        preset.enabled = shouldBeEnabled;
        banks[activeBankIndex].ccPresetStates[activePreset][slot] = shouldBeEnabled;

        ccToggleButtons[slot]
            .setToggleState(shouldBeEnabled, juce::dontSendNotification);
    }

    /* 4. Обновляем ccValue ТОЛЬКО если кнопка включена
          (т.е. параметр несёт осмысленное значение) */
    if (shouldBeEnabled)
    {
        const int newVal = invert ? (127 - v127) : v127;
        if (preset.ccValue != newVal)
            preset.ccValue = newVal;
    }

    /* 5. Перерисовать кнопку/лейблы */
    updatePresetButtons();
    repaint();
}
