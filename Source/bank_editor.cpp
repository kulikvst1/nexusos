...
void BankEditor::showVSTDialog()
{
    ...
    {
        // Получаем массив PluginEntry
        auto& entries = vstHost->getPluginManager().getPlugins();

        // Фильтруем только enabled==true и строим PopupMenu
        juce::PopupMenu menu;
        ...
        for (int i = 0; i < entries.size(); ++i)
        {
            ...
            menu.addItem(itemId, e.desc.name);
        ...
        if (indexMap.empty())
        {
            juce::AlertWindow::showMessageBoxAsync(
                ...
                "No Plugins Enabled",
                "Отметьте плагин(ы) в Plugin Manager.");
            return;
        }

        // Показать меню и загрузить выбранный плагин
        menu.showMenuAsync(
            juce::PopupMenu::Options().withTargetComponent(&vstButton),
            [this, indexMap](int choice)
            {
                if (choice <= 0 || choice > (int)indexMap.size())
                    return;

                // realIdx — индекс в entries[]
                int realIdx = indexMap[choice - 1];
                auto& desc = vstHost->getPluginManager()
                    .getPlugins().getReference(realIdx).desc;
                ...
                // Загрузить плагин
                vstHost->loadPlugin(
                    juce::File(desc.fileOrIdentifier),
                    sr, bs);
