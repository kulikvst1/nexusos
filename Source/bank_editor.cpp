...
void BankEditor::updateCCParameter(int index, bool /*ignoredState*/)
{
    ...
    // ──────────────────────────────────────── вариант 2
    /*
    if (!preset.invert)          // обычный режим
    {
        effective = preset.enabled ? preset.ccValue : 0;
    }
    else                         // инверсия кнопки + инверсия величины
    {
        const uint8_t invValue = 127 - preset.ccValue;    // числовое отражение
        effective = preset.enabled ? 0 : invValue;        // логическое отражение
    }
    */
    // MIDI
...
void BankEditor::onPluginParameterChanged(int paramIdx, float normalised)
{
    ...
    const int v127 = juce::jlimit(0, 127,
        static_cast<int> (std::round(normalised * 127.f)));

    bool touched = false;

    // 1. ищём, какая из 10 CC-кнопок привязана к этому параметру
    int ccSlot = -1;
    for (int i = 0; i < numCCParams; ++i)
    ...
    {
        const bool invert =
            banks[activeBankIndex].globalCCMappings[ccSlot].invert;

        auto& presetMap =
            banks[activeBankIndex]
            .presetCCMappings[activePreset][ccSlot];   // PresetCCMapping&

        const int newVal = invert ? (127 - v127) : v127;
        if (presetMap.ccValue != newVal)
        {
            presetMap.ccValue = newVal;
            touched = true;
        }
    }
    ...
    if (touched)
    {
        updatePresetButtons();   // пере-рисовать кнопки/слайдеры
        repaint();
    }
}
