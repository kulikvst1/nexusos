...
// метки 
    ...
    activeBankLabel.setColour(juce::Label::backgroundColourId, juce::Colours::black);
    activeBankLabel.setColour(juce::Label::textColourId, juce::Colours::white);
    ...
    activeLibraryLabel.setColour(juce::Label::backgroundColourId, juce::Colours::black);
    activeLibraryLabel.setColour(juce::Label::textColourId, juce::Colours::white);
...
void VSTHostComponent::resized()
{
    ...
    // Размеры меток: высота как кнопка пресета (30), ширина как две кнопки (200)
    int labelHeight = presetBtn[0].getHeight();
    int labelWidth = presetBtn[0].getWidth() * 3;

    // Левая метка (Library)
    activeLibraryLabel.setBounds(10, top.getY(), labelWidth, labelHeight);

    // Правая метка (Bank)
    activeBankLabel.setBounds(getWidth() - labelWidth - 10, top.getY(), labelWidth, labelHeight);


    // Plugin editor занимает оставшееся пространство
...
void VSTHostComponent::updateBPM(double newBPM)
{
    ...
    // обновляем playHead, если он есть
    if (playHead)
        playHead->setBpm(newBPM);

    // обновляем метку, если она привязана
    if (bpmLabel != nullptr)
    {
