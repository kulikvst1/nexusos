...
VSTHostComponent::VSTHostComponent(juce::AudioDeviceManager& adm): deviceMgr(adm)
{
    ...
    scanForPlugins();
    DBG("[Scanner] finished @" << juce::Time::getCurrentTime().toString(true, true));
...
void VSTHostComponent::layoutPluginEditor(Rectangle<int> area)
{
    ...
    // ─── защита: осталось ли место? ───
    if (area.getWidth() < w || area.getHeight() < h)
        return;                 // окно слишком мало – тихо выходим
...
void VSTHostComponent::resized()
{
    ...
    for (auto& b : presetBtn) pf.items.add(FlexItem(b).withMinWidth(100).withMinHeight(30));
    ...
    for (auto& b : learnBtn) lf.items.add(FlexItem(b).withMinWidth(120).withMinHeight(30));
