...
void VSTHostComponent::changeListenerCallback(juce::ChangeBroadcaster*)
{
    ...
    for (const auto& entry : pluginManager.getPluginsSnapshot())
        if (entry.enabled)
            pluginFiles.emplace_back(File(entry.getPath()));
...
void VSTHostComponent::addTuner(TunerComponent* t) noexcept
{
    ...
    // если ещё нет в списке — добавляем
    if (std::find(tuners.begin(), tuners.end(), t) == tuners.end())
    {
        tuners.push_back(t);
        processCb->setTuners(tuners);  // <-- новый метод в PluginProcessCallback
    }
}
...
{
    auto it = std::remove(tuners.begin(), tuners.end(), t);
    if (it != tuners.end())
    {
        tuners.erase(it, tuners.end());
        processCb->setTuners(tuners);
    }
}
//==============================================================================
//==============================================================================
