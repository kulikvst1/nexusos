...
#include <windows.h>
LooperComponent::LooperComponent(LooperEngine& eng, bool embedded) : engine(eng), embeddedInRig(embedded)
{
    ...
    {
        // остаются невидимыми, но живыми для triggerClick()
    ...
    //для цифровой клавы
    /*
    levelSlider.setLookAndFeel(&lnf);//цифр.клава
    mixSlider.setLookAndFeel(&lnf);//цифр.клава
    */
    startTimerHz(30);
...
void LooperComponent::sliderValueChanged(juce::Slider* s)
{
    ...
    if (s == &levelSlider)
    {
        float lev = (float)levelSlider.getValue();
        engine.setLevel(lev);

        if (onLevelChanged)
            onLevelChanged(lev); // уведомляем Rig_control
    }
    else if (s == &mixSlider)
    {
...
void LooperComponent::handleReset()
{
    engine.reset();
    progressSlider.setValue(0.0, juce::dontSendNotification);
    currentTimeLabel.setText("00:00", juce::dontSendNotification);
    ...
    updateStateLabel(); // единая точка обновления UI + MIDI
}

void LooperComponent::handleControl()
{
    if (engine.getMode() == LooperEngine::Mode::Looper)
    {
...
void LooperComponent::setLevelFromRig(float db)
{
    float gain = juce::Decibels::decibelsToGain(db);
    levelSlider.setValue(gain, juce::sendNotificationSync);
    engine.setLevel((float)levelSlider.getValue());
}
void LooperComponent::setLevelMidi(int value)
{
    float lev = juce::jmap<float>((float)value, 0.0f, 127.0f, 0.0f, 1.0f);
    levelSlider.setValue(lev, juce::sendNotificationSync);
    engine.setLevel(lev);
}
float LooperComponent::getLevelSliderValue() const noexcept
{
    return (float)levelSlider.getValue(); // возвращает текущее значение 0–1
}
