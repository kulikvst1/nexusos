...
public:
    ...
    void resized() override
    {
        ...
        if (pluginInstance != nullptr && pluginEditor != nullptr)
        {
            ...
            // размещаем 6 кнопок того же размера, что parametersButton (150)
            for (int i = 0; i < kNumPresets; ++i)
                presetBtn[i].setBounds(topBar.removeFromLeft(150).reduced(4));
    ...
    //----- PRESET helpers --------------------------------------------
    void setExternalPresetIndex(int idx);    //  вызывается BankEditor
