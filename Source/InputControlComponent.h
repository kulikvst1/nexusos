...
public:
    ...
    juce::Slider& getPedalThresholdSlider() { return pedalThresholdSlider; }
    juce::TextButton& getInvertButton() { return invertButton; }

    bool getInvertState() const { return invertButton.getToggleState(); }
    std::function<void(bool, bool)> onInputClipChanged;

    float getMeterLevelL() const noexcept { return vuLeftLevel.load(std::memory_order_relaxed); }
    float getMeterLevelR() const noexcept { return vuRightLevel.load(std::memory_order_relaxed); }
...
private:
    ...
    std::atomic<float> vuLeftLevel{ 0.0f };
    std::atomic<float> vuRightLevel{ 0.0f };
