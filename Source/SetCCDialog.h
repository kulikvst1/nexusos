...
struct CCMapping
{
    ...
    uint8_t       ccValue = 64;
    bool          invert = false;
    juce::String  name;             // ← добавлено для хранения отображаемого имени
};
...
public:
    ...
    void resized() override
    {
        ...
        enableToggle.setBounds(area.removeFromTop(45));
...
private:
    ...
    juce::ToggleButton enableToggle, invertToggle;
    ...
    void resetToDefaults()
    {
        ...
        mapping.ccValue = 64;
        mapping.name = "<none>";            // ← сброс поля name
    ...
    void finish(bool okPressed)
    {
        ...
        // ← обновляем поле name по выбранному параметру
        if (mapping.paramIndex >= 0 && host->getPluginInstance() != nullptr)
            mapping.name = host
            ->getPluginInstance()
            ->getParameterName(mapping.paramIndex);
        else
            mapping.name = "<none>";
