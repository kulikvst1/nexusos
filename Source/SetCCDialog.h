...
public:
    ...
    */
    ...
    {
        // Устанавливаем заголовок для окна (он появится в title bar)
        // и одновременно используем slotName для отображения внутри окна.
        ...
        centreWithSize(400, 240); // увеличенный размер для учета заголовка
    ...
    {
        // Получаем клиентскую область и задаём уменьшенный отступ (например, 5 пикселей)
        auto r = getLocalBounds().reduced(5);

        // Устанавливаем заголовок в самом верху: x = 5, y = 0, ширина - отступы слева и справа, высота = 40 пикселей
        titleLabel.setBounds(5, -8, r.getWidth() - 10, 40);

        // Теперь убираем ту же область, которую занял заголовок, чтобы разместить остальные элементы ниже
        r.removeFromTop(40);

        // Например, для ComboBox — 30 пикселей высоты:
        paramBox.setBounds(r.removeFromTop(30));

        // Toggle кнопка — 25 пикселей:
        enableToggle.setBounds(r.removeFromTop(25));

        // Слайдер — 50 пикселей:
        levelSlider.setBounds(r.removeFromTop(50));

        // Размещение кнопок OK/Cancel в нижней части
...
private:
    ...
    // Новый Label для отображения названия редактируемой кнопки в заголовке окна
