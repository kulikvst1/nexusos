...
public:
...

    {
        ...
        tapTempoDisplay.setEditable(true, true, false);
        tapTempoDisplay.onTextChange = [this]()
            {
                auto text = tapTempoDisplay.getText().trim();
                int bpm = text.getIntValue();

                if (bpm >= 40 && bpm <= 350)
                {
                    vstHostComponent->updateBPM(bpm);
