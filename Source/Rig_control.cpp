    ...
    enginePtr->onStateChanged = [this](LooperEngine::State s)
        {
            ...
            updateVolumeLabel(); // 🔹 обновляем метку при смене состояния
        };

    enginePtr->onCleared = [this]()
        {
            sendLooperClearToMidi();
            updateVolumeLabel(); // 🔹 при очистке Looper → OUT
        };
    ...
    enginePtr->onModeChanged = [this](LooperEngine::Mode m)
        {
            ...
            updateVolumeLabel(); // 🔹 при смене режима обновляем метку
        };
        ...
        looperComponent->onLevelChanged = [this](float lev)
            {
                ...
                sendVolumeToController(raw);
                updateVolumeLabel(); // 🔹 при изменении уровня тоже обновляем метку
            };
    }

    updateVolumeLabel(); // 🔹 начальная установка метки
}
