...
void Rig_control::resized()
{
    ...
    // 4.1) кнопка Looper – поднята на Row 2, Col 8 → сектор 17
    looperBtn.setBounds(getSectorRect(26).reduced(m2));

    // 4.2) если движок не установлен – прячем лупер и выходим
    ...
    // 4.3) ленивый create-инстанс LooperComponent
    if (!looperComponent)
    {
    ...
    // 4.4) показываем или прячем
    if (looperComponent->isVisible())
    {
        // во сколько раз «вдуть» внутренний 5×5-грид
        looperComponent->setScale(3.0f);

        // захватываем блок **один сектор вправо**: Col 3…8
        // верхняя часть – Row 2 cols 3…8 → индексы 12…17
        auto topRow = getUnionRect(11, 17).reduced(m2);
        // нижняя часть – Row 4 cols 3…8 → индексы 30…35
        auto bottomRow = getUnionRect(20, 25).reduced(m2);

        // комбинируем в один прямоугольник нужной высоты
        juce::Rectangle<int> looperArea(
            topRow.getX(),
            topRow.getY(),
            topRow.getWidth(),
            bottomRow.getBottom() - topRow.getY()
        );
        looperComponent->setBounds(looperArea);
