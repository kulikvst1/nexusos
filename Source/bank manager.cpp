...
BankManager::BankManager()
    : BankManager(nullptr)   // делегируем основной
{}
...
void BankManager::buttonClicked(juce::Button* b)
{
    ...
    // В BankManager::buttonClicked, когда обрабатывается нажатие кнопки "Set CC X"
    for (int slot = 0; slot < setCCButtons.size(); ++slot)
    {
        if (setCCButtons[slot] == b)
        {
            // Выбираем текущие настройки CC для этого слота
            auto initMap = banks[currentBankIndex].ccMappings[slot];

            // Получаем название кнопки – например, "Set CC 3"
            juce::String ccButtonName = setCCButtons[slot]->getButtonText();  // например, "Set CC 3"

            new SetCCDialog(vstHost,
                banks[currentBankIndex].ccMappings[slot],  // передаем один CCMapping, а не весь массив
                ccButtonName,
