...
OutControlComponent::OutControlComponent()
{
    ...
    for (int i = 0; i < 5; ++i)
    {
        ...
        for (int i = 0; i < 5; ++i)
        {
            ...
            if (i == 0) // Low-Cut
            {
                s.setRange(20.0, 1000.0, 1.0);   // допустим, от 20 до 1кГц
                s.setValue(30.0);                // стандартный HP @30 Hz
                s.setTextValueSuffix(" Hz");
            }
            else if (i == 4) // High-Cut
            {
                s.setRange(5000.0, 20000.0, 100.0); // 5…20 кГц
                s.setValue(16000.0);                // LP @16 кГц
                s.setTextValueSuffix(" Hz");
            }
            else
            {
                // i==1 → LowShelf, i==2 → Peak, i==3 → HighShelf
                s.setRange(-12.0, 12.0, 0.1);   // ±12 дБ
                s.setValue(0.0);                // 12 часов = 0 дБ
...
{   
    DBG("▶️ OutControlComponent::processAudioBlock called, ch=" << buffer.getNumChannels()
        << " samples=" << buffer.getNumSamples());
    DBG("   eqParamsDirty = " << (eqParamsDirty.load() ? "true" : "false"));
    ...
    {
        eqDsp.updateCoefficients(
            lowCutFreq,
            /* lowFreq */ 100.0f, lowGain,
            /* midFreq */ 1000.0f, midGain, 1.0f,
            /* highFreq */ 10000.0f, highGain,
            highCutFreq);
    }

    // 3) Прогоним через EQ
    eqDsp.process(buffer);//////////////////////////////////////////////////////////////////////////////////если что отключи                                                                                                                                   
