    ...
    void updateCCParameter(int index, bool state)
    {
        ...
        if (auto* bm = dynamic_cast<BankManager*>(bankTab.get()))
        {
            ...
            if (bankIdx >= 0 && bankIdx < bm->getBanks().size())
            {
                ...
                uint8_t effectiveValue = (slot.enabled && slot.paramIndex >= 0)
                    ? (state ? slot.ccValue : 0)
                    : (state ? 127 : 0);
                // Если установлен флаг инверсии, инвертируем значение: 127 - effectiveValue.
                if (slot.invert)
                    effectiveValue = 127 - effectiveValue;

                ccValue = effectiveValue;
            }
            else
            {
                ccValue = state ? 127 : 0;
        ...
        if (auto* vstHost = getVSTHostComponent())
        {
            if (auto* bm = dynamic_cast<BankManager*>(bankTab.get()))
            {
                ...
                {
                    const CCMapping& slot = bm->getBanks()[bankIdx].ccMappings[index];
                    // Обновляем плагин только если слот активен и параметр назначен.
                    if (slot.enabled && slot.paramIndex >= 0)
                    {
                        // Здесь также можно инвертировать значение, если требуется:
                        uint8_t pluginValue = ccValue; // ccValue уже инвертирован, если slot.invert
                        vstHost->setPluginParameter(slot.paramIndex, pluginValue);
                    }
                }
            }
        }
