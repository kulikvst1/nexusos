...
public:
    ...
    MultiMidiSenderComponent::MultiMidiSenderComponent()
    {
        ...
        // 2) Создаём BankManager _сразу_ с указателем на хост
        bankTab = std::make_unique<BankManager>(vstHostComponent.get());
        // 3) Остальные вкладки
        ...
        tabbedComponent->addTab("BANK MANAGER", juce::Colours::darkgrey, bankTab.get(), false);
    ...
    void updateCCParameter(int index, bool state)
    {
        ...
        if (auto* bm = dynamic_cast<BankManager*>(bankTab.get()))
        {
            int bankIdx = bm->getActiveBankIndex();
            if (bankIdx >= 0 && bankIdx < bm->getBanks().size())
            {
                const CCMapping& slot = bm->getBanks()[bankIdx].ccMappings[index];
                // Если слот включён и параметр привязан – используем значение из мэппинга,
                // иначе при активном состоянии равное 127
                ccValue = (slot.enabled && slot.paramIndex >= 0) ? (state ? slot.ccValue : 0) : (state ? 127 : 0);
            }
            else
            {
                ccValue = state ? 127 : 0;
        ...
        // Отправляем MIDI-сообщение (номера контроллеров начинаются с 1)
        if (midiOut != nullptr)
        {
            auto msg = juce::MidiMessage::controllerEvent(1, index + 1, static_cast<juce::uint8>(ccValue));
            handleOutgoingMidiMessage(msg);
        }

        // Обновляем параметр плагина:
        if (auto* vstHost = getVSTHostComponent())
        {
            if (auto* bm = dynamic_cast<BankManager*>(bankTab.get()))
            {
                ...
                {
                    const CCMapping& slot = bm->getBanks()[bankIdx].ccMappings[index];
                    // Вариант A: обновляем плагин только если слот активен и параметр назначен.
                    if (slot.enabled && slot.paramIndex >= 0)
                    {
                        vstHost->setPluginParameter(slot.paramIndex, static_cast<juce::uint8>(ccValue));
                    }
                    // Иначе не обновляем плагин для этого слота.
                }
                else
                {
                    // Если не удалось получить банк, не обновляем.
                }
            }
            else
            {
                // Если не удалось привести bankTab к BankManager, не делаем обновление.
            }
        }
    }
    ...
    void buttonClicked(juce::Button* button) override
    {
        ...
        for (int i = 0; i < ccButtons.size(); ++i)
        {
            ...
            {
                // Получаем состояние кнопки (включено/выключено)
                bool state = button->getToggleState();

                // Унифицированное обновление CC-параметра:
                updateCCParameter(i, state);

                // Обновляем массив CC-Preset-States, если выбран какой-либо пресет
                ...
                if (selectedPreset >= 0)
                {
                    ...
                    if (auto* bm = dynamic_cast<BankManager*>(bankTab.get()))
                    {
                        ...
                        if (bankIdx >= 0 && bankIdx < bm->getBanks().size())
                        {
                            bm->getBanks()[bankIdx].ccPresetStates[presetIndex][i] = state;
                            // Сохраняем настройки (либо запускаем авто-сохранение)
                            bm->saveSettings();
                            // При необходимости можно сразу загрузить пресет:
                            // bm->loadPreset(presetIndex);
                        }
                    }
                }
                return;  // Кнопка обработана – выходим из цикла
            }
        }
        ...
        if (button->getRadioGroupId() == 100)
        {
            ...
            if (clickedIndex != -1 && button->getToggleState())
            {
                if (auto* bm = dynamic_cast<BankManager*>(bankTab.get()))
                {
                    ...
                    {
                        // Вычисляем индекс пресета с учётом состояния SHIFT
                        ...
                        // Обновляем CC-кнопки по единой логике: устанавливаем состояние и вызываем updateCCParameter
                        for (int j = 0; j < ccButtons.size(); ++j)
                        {
                            ...
                            updateCCParameter(j, presetState);
                        }
                        ...
                        // Обновляем volumeSlider для выбранного пресета
                        if (auto* bm = dynamic_cast<BankManager*>(bankTab.get()))
                        {
                            int activeIndex = bm->getActiveBankIndex();
                            if (activeIndex >= 0 && activeIndex < bm->getBanks().size())
                            {
                                volumeSlider->setValue(bm->getBanks()[activeIndex].presetVolumes[presetIndex],
                                    juce::dontSendNotification);
                            }
                            int newVolume = static_cast<int>(volumeSlider->getValue());
                            if (auto* vstHost = getVSTHostComponent())
                            {
                                vstHost->setPluginParameter(volumeCCNumber, static_cast<juce::uint8>(newVolume));
                            }
                        }
                    }
                }
...
private:
    ...
    std::unique_ptr<BankManager>           bankTab;           // ↓ потом
