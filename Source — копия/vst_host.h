...
//+++++++++++++++++++
struct EmojiOnlyLookAndFeel : public juce::LookAndFeel_V4
{
    void drawButtonBackground(juce::Graphics&, juce::Button&, const juce::Colour&, bool, bool) override
    {
        // ничего не рисуем → фон и рамка исчезают
    }

    void drawButtonText(juce::Graphics& g, juce::TextButton& button,
        bool, bool) override
    {
        auto font = juce::Font(40.0f, juce::Font::plain); // размер эмодзи
        g.setFont(font);
...
class BankEditor;
class RoundedBackground : public juce::Component
{
public:
    void paint(juce::Graphics& g) override
    {
        ...
        g.fillRoundedRectangle(bounds, 12.0f); // радиус скругления 12px
    }
};
...
public:
    //==============================================================================
    // Теперь конструктор принимает ссылку на PluginManager
    ...
    void setBpmDisplayEditor(juce::TextEditor* editor) noexcept;
    ...
    void setFilePlayerEngine(FilePlayerEngine* engine) noexcept;
    ...
    void closePluginEditorIfOpen();
    void openPluginEditorIfNeeded();
    bool isPluginEditorOpen() const noexcept;
    void setBankEditor(BankEditor* editor);
    void updateCCLabel(int slot, const juce::String& text); // 👈 новый метод
    void setCCButtonState(int slot, bool active);
    std::function<void(int slot, bool state)> onCCButtonClicked;
    
    void updateActiveLibraryBankLabel(BankEditor* bankEditor);
private:
    // Internal helpers
    ...
    juce::TextEditor* bpmEditor = nullptr;


    // Plugin manager (теперь ссылка на глобальный)
    ...
    FilePlayerEngine* filePlayerEngine = nullptr;
    ...
    juce::Label presetBackground;
    RoundedBackground bottomBackground;
    bool learnBlockVisible = true; // по умолчанию показываем
    juce::TextButton toggleLearnBlockButton;
    EmojiOnlyLookAndFeel bigIconLF;
    juce::Rectangle<int> normalEditorBounds;
    juce::Rectangle<int> expandedEditorBounds;
