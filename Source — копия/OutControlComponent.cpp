...
void OutControlComponent::timerCallback()
{
    ...
    if (onClipChanged)
        onClipChanged(clipLedL.isVisible(), clipLedR.isVisible());
...
OutControlComponent::OutControlComponent()
{
    ...
    for (int i = 0; i < 5; ++i)
    {
        ...
        eqSliders[i].setTextBoxStyle(juce::Slider::TextBoxBelow, false, 60, 20);//or TextBoxBelow or TextBoxAbove
        eqSliders[i].setColour(juce::Slider::textBoxTextColourId, juce::Colours::black);//цвет шрифта
        eqSliders[i].setColour(juce::Slider::textBoxBackgroundColourId, juce::Colours::white);//цвет рамки
    ...
    gainSliderL.setTextBoxStyle(juce::Slider::TextBoxAbove, false, 60, 20);
    gainSliderL.setColour(juce::Slider::textBoxTextColourId, juce::Colours::black);//цвет шрифта
    gainSliderL.setColour(juce::Slider::textBoxBackgroundColourId, juce::Colours::white);//цвет рамки
    ...
    gainSliderR.setTextBoxStyle(juce::Slider::TextBoxAbove, false , 60, 20);
    gainSliderR.setColour(juce::Slider::textBoxTextColourId, juce::Colours::black);//цвет шрифта
    gainSliderR.setColour(juce::Slider::textBoxBackgroundColourId, juce::Colours::white);//цвет рамки
    ...
    //---- Метки активного канала 
    addAndMakeVisible(activeSquareL);
    activeSquareL.setText(" ", juce::dontSendNotification); // ← пробел, не пусто
    activeSquareL.setColour(juce::Label::backgroundColourId, juce::Colours::black);
    activeSquareL.setColour(juce::Label::textColourId, juce::Colours::transparentBlack);
    activeSquareL.setOpaque(true);

    addAndMakeVisible(activeSquareR);
    activeSquareR.setText(" ", juce::dontSendNotification);
    activeSquareR.setColour(juce::Label::backgroundColourId, juce::Colours::black);
    activeSquareR.setColour(juce::Label::textColourId, juce::Colours::transparentBlack);
    activeSquareR.setOpaque(true);
    ...
    // 🔹 сразу уведомляем внешний компонент о начальном состоянии
    if (onLinkStateChanged)
        onLinkStateChanged(linkButton.getToggleState());

    // обновляем atomicLink при клике на кнопку
    linkButton.onClick = [this]()
        {
            bool state = linkButton.getToggleState();
            atomicLink.store(state, std::memory_order_relaxed);

            if (onLinkStateChanged)
                onLinkStateChanged(state); // уведомляем внешний компонент
        };
    ...
    addAndMakeVisible(pluginSlotBox);
...
OutControlComponent::OutControlComponent(PluginManager* pm, bool vstOnly)
    : pluginManager(pm)
{
    formatManager.addDefaultFormats();

    // Те же самые элементы должны быть добавлены
    addAndMakeVisible(pluginSlotBox);
    addAndMakeVisible(editPluginButton);
    addAndMakeVisible(removePluginButton);

    pluginSlotBox.addListener(this);
    ...
    pluginSlotBox.onChange = [this]() { handlePluginSelection(); };
    editPluginButton.onClick = [this]() { editPlugin(); };
    removePluginButton.onClick = [this]() { unloadPlugin(); };
}
...
{    
    ...
    // 6.3) метки активного канала 
    {
        auto lbL = gainLabelL.getBounds();
        int sqSize = SY(20);
        activeSquareL.setBounds(lbL.getRight() - sqSize - SX(4),
            lbL.getY() + SY(4),
            sqSize, sqSize);

        auto lbR = gainLabelR.getBounds();
        activeSquareR.setBounds(lbR.getX() + SX(4),   // 🔹 левый угол метки
            lbR.getY() + SY(4),
            sqSize, sqSize);
    }
    ...
    auto vstArea = grid.getSector(2, 0)
        .getUnion(grid.getSector(3, 0))
        .reduced(SX(8), SY(8));

    pluginSlotBox.setBounds(vstArea.removeFromTop(SY(24))); // ComboBox пониже
    auto btnArea = vstArea.removeFromTop(SY(24));
    editPluginButton.setBounds(btnArea.removeFromLeft(SX(60))); // кнопки меньше
    removePluginButton.setBounds(btnArea.removeFromLeft(SX(60)));
...
void OutControlComponent::sliderValueChanged(juce::Slider* s)
{
    ...
    // 🔹 уведомляем Rig_control, какой слайдер изменился
    if (onGainSliderChanged)
        onGainSliderChanged(s);
...
void OutControlComponent::setLinkState(bool on)
{
    linkButton.setToggleState(on, juce::sendNotification);
    atomicLink.store(on, std::memory_order_relaxed);
    updateActiveSquares(nullptr);
}

bool OutControlComponent::getLinkState() const noexcept
{
    return linkButton.getToggleState();
}
void OutControlComponent::updateActiveSquares(juce::Slider* s)
{
    if (atomicLink.load())
    {
        setActiveSquares(true, true);
    }
    else
    {
        if (s == &gainSliderL)
            setActiveSquares(true, false);
        else if (s == &gainSliderR)
            setActiveSquares(false, true);
    }
}
void OutControlComponent::setActiveSquares(bool leftOn, bool rightOn)
{
    activeSquareL.setVisible(leftOn);
    activeSquareR.setVisible(rightOn);

    if (leftOn)
        activeSquareL.setColour(juce::Label::backgroundColourId, juce::Colours::blue);

    if (rightOn)
        activeSquareR.setColour(juce::Label::backgroundColourId, juce::Colours::blue);
}


void OutControlComponent::offsetGainDb(float deltaDb) noexcept
{
    if (!atomicLink.load(std::memory_order_relaxed))
        return; // 🔹 теперь offsetGainDb работает только при включённом Link
    ...
    gainValL.store(juce::Decibels::decibelsToGain(newL), std::memory_order_relaxed);
    gainValR.store(juce::Decibels::decibelsToGain(newR), std::memory_order_relaxed);
...
void OutControlComponent::saveSettings() const
{
    ...
    // 3) Запись в файл в Roaming\NEXUS_KONTROL_OS
    ...
    auto file = settingsDir.getChildFile(settingsFileName);
    xml->writeTo(file, {});
}
...
float OutControlComponent::getMeterLevelL() const noexcept
{
    return atomicMeterL.load(std::memory_order_relaxed);
}

float OutControlComponent::getMeterLevelR() const noexcept
{
    return atomicMeterR.load(std::memory_order_relaxed);
}
//++++++++++++++++++++++++++++++
void OutControlComponent::changeListenerCallback(juce::ChangeBroadcaster* source)
{
    if (source == pluginManager)
        refreshPluginList();
}

void OutControlComponent::comboBoxChanged(juce::ComboBox* comboBox)
{
    if (comboBox == &pluginSlotBox)
        handlePluginSelection();
}

void OutControlComponent::refreshPluginList()
{
    pluginSlotBox.clear();

    if (pluginManager)
    {
        auto snapshot = pluginManager->getPluginsSnapshot();
        int itemId = 1;
        for (int i = 0; i < snapshot.size(); ++i)
        {
            ...
            pluginSlotBox.addItem(snapshot[i].getName(), itemId++);
        }
    }

    // тестовый fallback
    if (pluginSlotBox.getNumItems() == 0)
    {
        pluginSlotBox.addItem("No plugins found", 1);
    }

    pluginSlotBox.setSelectedId(1, juce::dontSendNotification);
}


void OutControlComponent::handlePluginSelection()
{
    int idx = pluginSlotBox.getSelectedId() - 1;
    if (idx < 0 || !pluginManager) return;
    ...
    if (idx >= (int)indexMap.size()) return;

    const auto& descr = snapshot[indexMap[idx]].desc;
    juce::String error;
    auto instance = formatManager.createPluginInstance(descr, 44100.0, 512, error);
    if (instance)
    {
    ...
    {
        DBG("Ошибка загрузки: " + error);
    }
}

void OutControlComponent::editPlugin()
{
    if (pluginInstance)
    {
...
void OutControlComponent::loadPlugin(const juce::File& file)
{
    ...
    if (!format)
    {
        DBG("Нет доступных форматов");
        return;
    }

    juce::OwnedArray<juce::PluginDescription> descr;
    format->findAllTypesForFile(descr, file.getFullPathName());

    if (descr.isEmpty())
    {
        DBG("Не удалось получить описание");
        return;
    }

    juce::String error;
    auto instance = format->createInstanceFromDescription(*descr[0], 44100.0, 512, error);
...
void OutControlComponent::unloadPlugin()
{
    if (pluginInstance)
    {
    ...
    pluginSlotBox.setSelectedId(0, juce::dontSendNotification);
}
