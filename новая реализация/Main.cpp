...
public:
    ...
    void updateCCParameter(int index, bool state)
    {
        ...
        // Отправляем MIDI-сообщение: нумерация кнопок от 1
        if (midiOut != nullptr)
        {
        ...
        // Обновляем параметр плагина (через слот мэппинга, если он задан)
            ...
            if (auto* bm = dynamic_cast<BankManager*>(bankTab.get()))
            {
                ...
                if (bankIdx >= 0 && bankIdx < bm->getBanks().size())
                {
                    ...
                    if (slot.enabled && slot.paramIndex >= 0)
                        ...
                        vstHost->setPluginParameter(index, static_cast<juce::uint8>(ccValue));
                }
                else
                {
                    vstHost->setPluginParameter(index, static_cast<juce::uint8>(ccValue));
