...
void BankEditor::setActivePreset(int newPreset)
{
    ...
    for (int i = 0; i < numCCParams; ++i)
    {
        ...
        ccToggleButtons[i].setToggleState(state,
            juce::dontSendNotification);
        updateCCParameter(i, state);
    }
...
void BankEditor::timerCallback()
{
    ...
    if (vstHost->getPluginScanJobCount() < 1)
        vstHost->addPluginScanJob(new SaveJob(*this),
            true /* autoDelete */);
}
...
{
    // Скрываем любые активные PopupMenu
    juce::PopupMenu::dismissAllActiveMenus();
    if (vstHost == nullptr)
        return;

    // Ветка A: плагин еще не загружен
    if (vstHost->getPluginInstance() == nullptr)
    {
        // Получаем вектор файлов плагинов
        ...
        menu.showMenuAsync(
            ...
            [this](int choice)
            {
                ...
                // Выбрали пункт — вновь берём список файлов
                const auto& files2 = vstHost->getPluginFiles();
                auto fileToLoad = files2[size_t(choice - 1)];

                // Получаем текущие аудио-настройки
                auto& adm = vstHost->getAudioDeviceManagerRef();
                auto* dev = adm.getCurrentAudioDevice();
                const double sr = dev ? dev->getCurrentSampleRate() : 44100.0;
                const int    bs = dev ? dev->getCurrentBufferSizeSamples() : 512;

                // Загружаем плагин
                vstHost->loadPlugin(fileToLoad, sr, bs);

                // Обновляем маппинг CC-параметров под новый плагин
                auto* inst = vstHost->getPluginInstance();
                const int nParams = inst
                    ? static_cast<int>(inst->getParameters().size())
                    : 0;

                for (int s = 0; s < numCCParams; ++s)
                {
                    auto& map = banks[activeBankIndex].globalCCMappings[s];
                    if (map.paramIndex < 0 || map.paramIndex >= nParams)
                    {
                        map.paramIndex = -1;
                        map.name = "<none>";
                    }
    ...
    // Ветка B: плагин уже загружен — спрашиваем, надо ли закрыть
    juce::AlertWindow::showOkCancelBox(
        ...
        juce::ModalCallbackFunction::create([this](int result)
            {
                // В JUCE обычно result == 1 означает «Ok»
                if (result == 1)
                {
