...
void Rig_control::setBankEditor(BankEditor* editor)
{
    ...
    // сохраняем старый коллбэк пресетов (если был)
    auto prevPresetCB = bankEditor->onActivePresetChanged;
    // реагируем на смену пресета: подсветка Shift + пересборка UI
    bankEditor->onActivePresetChanged = [this, prevPresetCB](int presetIndex)
        {
            if (prevPresetCB)
                prevPresetCB(presetIndex);
    ...
    // реагируем на смену банка: сбрасываем Shift и пересобираем UI
