...
void BankManager::saveSettingsToFile(const juce::File& configFile)
{
    ...
    for (int i = 0; i < (int)banks.size(); i++)
    {
        ...
        // Сохраним массив состояний плагина как одну строку с разделителем, например, ";"
        juce::String combinedPresetStates;
        for (int j = 0; j < (int)banks[i].pluginPresetStates.size(); j++)
        {
            combinedPresetStates += banks[i].pluginPresetStates[j];
            if (j < banks[i].pluginPresetStates.size() - 1)
                combinedPresetStates += ";";
        }
        propertiesFile.setValue("bank_" + juce::String(i) + "_pluginPresetStates", combinedPresetStates);

        // Сохраняем путь к загруженному плагину
        propertiesFile.setValue("bank_" + juce::String(i) + "_loadedPluginPath", banks[i].loadedPluginPath);
    }
...
void BankManager::loadSettingsFromFile(const juce::File& configFile)
{
    ...
    for (int i = 0; i < (int)banks.size(); i++)
    {
        ...
        // *** Новые поля для плагина ***
        // Восстанавливаем состояния плагина:
        juce::String combinedPresetStates = propertiesFile.getValue("bank_" + juce::String(i) + "_pluginPresetStates", "");
        if (combinedPresetStates.isNotEmpty())
        {
            juce::StringArray tokens;
            tokens.addTokens(combinedPresetStates, ";", "");
            banks[i].pluginPresetStates.resize(tokens.size());
            for (int j = 0; j < tokens.size(); j++)
            {
                banks[i].pluginPresetStates[j] = tokens[j];
            }
        }
        // Восстанавливаем путь к загруженному плагину
        banks[i].loadedPluginPath = propertiesFile.getValue("bank_" + juce::String(i) + "_loadedPluginPath", banks[i].loadedPluginPath);
    }
...
void BankManager::loadPreset(int presetIndex)
{
    if (currentBankIndex >= 0 && currentBankIndex < banks.size())
    {
        juce::String stateString = banks[currentBankIndex].pluginPresetStates[presetIndex];
        if (stateString.isNotEmpty() && vstHostComponent != nullptr && vstHostComponent->getPluginInstance() != nullptr)
        {
            juce::MemoryBlock pluginState;
            pluginState.fromBase64Encoding(stateString);
            vstHostComponent->getPluginInstance()->setStateInformation(pluginState.getData(), (int)pluginState.getSize());
        }
    }
}
