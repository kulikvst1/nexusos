...
void BankEditor::updateUI()
{
    ...
    for (int i = 0; i < numCCParams; ++i)
    {
        ccNameEditors[i].setText(bank.globalCCMappings[i].name, juce::dontSendNotification);
        ccToggleButtons[i].setToggleState(bank.presetCCMappings[activePreset][i].enabled,
            juce::dontSendNotification);
    }
}
