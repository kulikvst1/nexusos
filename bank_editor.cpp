    ...
    : pluginManager(pm), vstHost(host), shouldLoadDefaultOnStartup(loadDefaultFlag)
{
  ...
  //  bankIndexLabel.setColour(juce::Label::backgroundColourId, btnBg);
    ...
    {
        slotPresetCCMappings[s].resize(numPresets);
        for (auto& ccArray : slotPresetCCMappings[s])
    ...
    {
        vstButtons[i].setRadioGroupId(1234);
        vstButtons[i].setClickingTogglesState(true);
        vstButtons[i].setButtonText("VST" + juce::String(i + 1));
        vstButtons[i].setColour(juce::TextButton::buttonOnColourId, juce::Colours::yellow);

        vstButtons[i].onClick = [this, i]()
            {
                activeSlot = i;

                if (auto* inst = vstHost->getPluginInstance(activeSlot))
                {
                    vstHost->showPluginEditor(i); // показать редактор слота
                    pluginLabel.setText(inst->getName(), juce::dontSendNotification);
                    vstButton.setButtonText(inst->getName()); // кнопка загрузки показывает имя
                }
                else
                {
                    // слот пустой → закрываем редактор
                    vstHost->closePluginEditorIfOpen();

                    pluginLabel.setText("<no plugin>", juce::dontSendNotification);
                    vstButton.setButtonText("Load VST");
                }
            };

        addAndMakeVisible(vstButtons[i]);
    }

    for (int i = 0; i < 4; ++i)
    {
        bypassButtons[i].setButtonText("BP " + juce::String(i + 1));
        bypassButtons[i].setClickingTogglesState(true);
        bypassButtons[i].setColour(juce::TextButton::buttonOnColourId, juce::Colours::red);
    ...
    banks.assign(numBanks, Bank{});//_______________________________________________________________________
...
void BankEditor::paint(juce::Graphics& g)
{
    ...
    // линия через VST кнопки
    if (vstButtons[0].isVisible())
    {
        int y = vstButtons[0].getY() + vstButtons[0].getHeight() / 2;

        // берём координаты первой и последней кнопки
        int x1 = vstButtons[0].getX();
        int x2 = vstButtons[3].getRight();

        // делаем линию длиннее на 1/4 ширины кнопки
        int extend = vstButtons[0].getWidth() / 4;

        g.setColour(juce::Colours::yellow);
        g.drawLine((float)(x1 - extend), (float)y, (float)(x2 + extend), (float)y, 2.0f);
    }
}void BankEditor::resized()
{
    ...
    // Row 3 → теперь Row 4 (VST + BYPASS)
    {
        ...
        // размеры как у presetButtons
        int bw = W / 8 - gap;
        int rowHeight = int(sH * shrinkFactor);

        // центрируем VST кнопки
        int totalWidth = 4 * bw + 3 * (bw / 2);
        int startX = baseX + (W - totalWidth) / 2;
        int yVST = baseY + (3 + rowOffset) * sH;

        for (int i = 0; i < 4; ++i)
        {
            int x = startX + i * (bw + bw / 2);
            vstButtons[i].setBounds(x, yVST, bw, rowHeight);
        }
        // VST + BYPASS row
        {
            int bw = W / 8 - gap;
            ...
            for (int i = 0; i < 4; ++i)
            {
                int xVST = startX + i * (bw + bw / 2);
                vstButtons[i].setBounds(xVST, yVST, bw, rowHeight);

                // bypass квадрат внутри VST‑кнопки, симметрично
                int bypassSize = int(rowHeight * 0.85f);
                int margin = (rowHeight - bypassSize) / 2; // одинаковый отступ сверху, снизу и слева
                int xBP = xVST + margin;
                int yBP = yVST + margin;
                bypassButtons[i].setBounds(xBP, yBP, bypassSize, bypassSize);
            }
        }
...
void BankEditor::buttonClicked(juce::Button* b)
{
    ...
    for (int i = 0; i < numCCParams; ++i)
    {
        if (b == &ccToggleButtons[i])
        {
            ...
            // обновляем модель для активного слота и пресета
            auto& mapping = slotPresetCCMappings[activeSlot][activePreset][i];
            mapping.enabled = state;

            // применяем к железу/плагину
    ...
    for (int i = 0; i < numCCParams; ++i)
    {
        if (b == &learnButtons[i])
        {
            ...
            // используем стандартный метод, хост сам знает activeSlot
...
void BankEditor::editCCParameter(int ccIndex)
{
    ...
    new SetCCDialog(vstHost, initialMap, slotName,
        [this, ccIndex](CCMapping newMap, bool ok)
        {
            ...
            {
                if (auto* inst = vstHost->getActivePluginInstance()) // теперь берём активный слот
...
void BankEditor::updateUI()
{
    ...
    // 🔽 теперь CC‑элементы берём из slotPresetCCMappings для активного слота
    for (int i = 0; i < numCCParams; ++i)
    {
        auto& mapping = slotPresetCCMappings[activeSlot][activePreset][i];

        ccNameEditors[i].setText(mapping.enabled ? "ON" : "OFF", juce::dontSendNotification);
        ccToggleButtons[i].setToggleState(mapping.enabled, juce::dontSendNotification);

        // если нужно подсветить learn‑кнопку
        learnButtons[i].setColour(
            juce::TextButton::buttonColourId,
            mapping.enabled ? juce::Colours::green : juce::Colours::darkgrey
        );
    }
}
void BankEditor::setActiveSlot(int slotIndex)
{
...
void BankEditor::loadSettingsFromFile(const juce::File& file)
{
    ...
    if (vstHost != nullptr && vstHost->getActivePluginInstance() != nullptr)
    {
        auto* inst = vstHost->getActivePluginInstance();
        ...
        if (newName.isEmpty() || newId.isEmpty())
        {
            ...
            vstHost->unloadPlugin(vstHost->getActiveSlotIndex());
        }
        ...
        {
            vstHost->loadPlugin(juce::File(globalPluginId),
                vstHost->getActiveSlotIndex(),
                vstHost->getCurrentSampleRate(),
                vstHost->getCurrentBlockSize());
    ...
    if (auto* inst = vstHost ? vstHost->getActivePluginInstance() : nullptr)
        pluginLabel.setText(inst->getName(), juce::dontSendNotification);
...
void BankEditor::resetAllDefaults()
{
    ...
    // 🔹 4) Выгружаем плагин именно из выбранного слота
    if (vstHost != nullptr)
        vstHost->unloadPlugin(activeSlot);
...
void BankEditor::showVSTDialog()
{
    ...
    // A: выбранный слот пуст → строим меню загрузки
    if (vstHost->getPluginInstance(activeSlot) == nullptr)
    {
        ...
        menu.showMenuAsync(
            ...
            [this, entries, indexMap](int choice)
            {
                ...
                vstHost->loadPlugin(juce::File(normalizePluginId(desc.fileOrIdentifier)),
                    activeSlot, sr, bs);

                auto* inst = vstHost->getPluginInstance(activeSlot);
    ...
    // B: выбранный слот уже занят → спросить о закрытии
    juce::AlertWindow::showOkCancelBox(
...
void BankEditor::onPluginParameterChanged(int paramIdx, float normalised)
{
    ...
    if (auto* inst = (vstHost != nullptr ? vstHost->getActivePluginInstance() : nullptr))
...
void BankEditor::toggleCC(int ccIndex, bool state)
{
    ...
    if (activePreset < 0 || activePreset >= numPresets) return;
    if (ccIndex < 0 || ccIndex >= numCCParams) return;

    // 1. Обновляем модель для конкретного слота
    auto& mapping = slotPresetCCMappings[activeSlot][activePreset][ccIndex];
    mapping.enabled = state;

    // 2. Обновляем UI
    ccToggleButtons[ccIndex].setToggleState(state, juce::dontSendNotification);

    // 3. Применяем к железу/плагину
...
void BankEditor::applyBankToPlugin(int bankIndex, bool synchronous /* = false */)
{
    ...
    auto* currentInst = vstHost->getPluginInstance(activeSlot); // берём выбранный слот
    bool needLoad = false;
    ...
    {
        vstHost->unloadPlugin(activeSlot); // выгружаем именно выбранный слот
        needLoad = true;
    }
    ...
    if (needLoad)
    {
        ...
        if (pluginDir.exists())
        {
            ...
            lastLoadedPluginId = b.pluginId; // фиксируем идентификатор
        }
        else
        {
    ...
    auto* instNow = vstHost->getPluginInstance(activeSlot); // снова выбранный слот
    if (!instNow)
        return;
        ...
        auto applyState = [this, targetId, stateCopy]()
            {
                ...
                auto* instCheck = vstHost->getPluginInstance(activeSlot);
                if (!instCheck) return;
                    ...
                    juce::Timer::callAfterDelay(80, [this, targetId]()
                        {
                            ...
                            if (vstHost->getPluginInstance(activeSlot))
...
void BankEditor::snapshotCurrentBank()
{
    ...
    auto* inst = vstHost->getActivePluginInstance(); // теперь берём активный слот
    if (!inst)
        return;
...
{
    // 1. Выгружаем сам плагин из выбранного слота
    if (vstHost)
        vstHost->unloadPlugin(activeSlot);
...
{
    if (vstHost != nullptr)
        if (auto* inst = vstHost->getPluginInstance(activeSlot)) // берём выбранный пользователем слот
...
void BankEditor::applyPedalValue(int slot, float norm)
{
    ...
    if (m.paramIndex >= 0)
    {
        if (auto* plug = vstHost->getPluginInstance(activeSlot)) // берём выбранный слот
        {
...
void BankEditor::updateVSTButtonLabel()
{
    ...
    if (vstHost != nullptr && vstHost->getPluginInstance(activeSlot) != nullptr)
