...
{
public:
    // Геттер возвращает корректный указатель на VSTHostComponent
    VSTHostComponent* getVSTHostComponent() { return vstHostComponent.get(); }
...

    {
        ...
        vstHostComponent = std::make_unique<VSTHostComponent>(); // создаем единожды!
        // Затем, если BankManager имеет метод setVSTHostComponent:
        dynamic_cast<BankManager*>(bankTab.get())->setVSTHostComponent(vstHostComponent.get(), juce::File(/* ваш configFile */));


        // Добавляем вкладки
        tabbedComponent->addTab("RIG KONTROL",
            juce::LookAndFeel::getDefaultLookAndFeel().findColour(juce::ResizableWindow::backgroundColourId),
            mainTab.get(),
            true);

        tabbedComponent->addTab("BANK MANAGER",
            juce::LookAndFeel::getDefaultLookAndFeel().findColour(juce::ResizableWindow::backgroundColourId),
            bankTab.get(),
            false);

        tabbedComponent->addTab("VST HOST",
            juce::LookAndFeel::getDefaultLookAndFeel().findColour(juce::ResizableWindow::backgroundColourId),
            vstHostComponent.get(),
            false);
        ...
        startTimerHz(0.5); // Опрашивать 2 раза в секунду
        ...
        // cpuLoadIndicator = std::make_unique<CpuLoadIndicator>(&globalCpuLoad);
    ...
    void buttonClicked(juce::Button* button) override
    {
        ...
        if (button == upButton.get() || button == downButton.get())
        {
            ...
            if (auto* bm = dynamic_cast<BankManager*>(bankTab.get()))
            {
                ...
                // Извлекаем активный банк и его настройку номера плагина-пресета
                const BankManager::Bank& activeBank = bm->getBank(newIndex);
                int pluginPresetNumber = activeBank.pluginPreset;

                // Если плагин загружен, передаем ему номер пресета из менеджера
                if (vstHostComponent != nullptr && vstHostComponent->getPluginInstance() != nullptr)
                {
                    int totalPresets = vstHostComponent->getPluginInstance()->getNumPrograms();
                    if (totalPresets > 0 && pluginPresetNumber >= 0 && pluginPresetNumber < totalPresets)
                    {
                        vstHostComponent->getPluginInstance()->setCurrentProgram(pluginPresetNumber);
                        DBG("Applied plugin preset from Bank Manager: " + juce::String(pluginPresetNumber));
                    }
                    else
                    {
                        DBG("The stored plugin preset number is out of range (or plugin has no programs).");
                    }
                }
            }
            return;
        ...
        // Обработка нажатия CC-кнопок
        for (int i = 0; i < ccButtons.size(); ++i)
        {
            if (ccButtons[i] == button)
            {
                bool state = button->getToggleState();
                // Если кнопка включена, то значение CC равно 127, иначе 0
                int ccValue = state ? 127 : 0;
                // Для MIDI-сообщения используем номер кнопки i+1
                int ccNumber = i + 1;

                // Отправка MIDI-сообщения
                if (midiOut != nullptr)
                {
                    auto msg = juce::MidiMessage::controllerEvent(1, ccNumber, static_cast<juce::uint8>(ccValue));
                    handleOutgoingMidiMessage(msg);
                }

                // Изменение параметра VST-плагина:
                // Вместо использования индекса i напрямую, получаем номер контроллера из настроек текущего банка
                if (auto* vstHost = getVSTHostComponent())
                {
                    // Пытаемся получить BankManager через bankTab
                    if (auto* bm = dynamic_cast<BankManager*>(bankTab.get()))
                    {
                        int activeBankIndex = bm->getActiveBankIndex();
                        if (activeBankIndex >= 0 && activeBankIndex < bm->getBanks().size())
                        {
                            // Используем значение из ccMapping для кнопки с индексом i  
                            int mappedCC = bm->getBanks()[activeBankIndex].ccMapping[i];
                            vstHost->setPluginParameter(mappedCC, ccValue);
                        }
                        else
                        {
                            // На всякий случай, если возникла проблема с активным банком, используем индекс i
                            vstHost->setPluginParameter(i, ccValue);
                        }
                    }
                    else
                    {
                        // Если bankTab не содержит BankManager, используем исходное значение
                        vstHost->setPluginParameter(i, ccValue);
                    }
                }

                // Определяем группу и выбранный пресет для обновления сохранённого состояния
                ...
                if (selectedPreset != -1)
                {
                    ...
                    if (auto* bm = dynamic_cast<BankManager*>(bankTab.get()))
                    {
                        ...
                        if (activeIndex >= 0 && activeIndex < bm->getBanks().size())
                        {
                            ...
                            // Вызываем сохранение настроек, чтобы изменения сразу записывались
                            bm->saveSettings(); // Можно заменить на bm->restartAutoSaveTimer();

                            // NEW: Вызываем метод загрузки сохранённого состояния плагина
                            bm->loadPreset(presetIndex);
                        }
                    }
                }
                return;
                ...
                if (auto* bm = dynamic_cast<BankManager*>(bankTab.get()))
                {
                    ...
                    if (activeIndex >= 0 && activeIndex < bm->getBanks().size())
                    {
                        ...
                        for (int j = 0; j < ccButtons.size(); ++j)
                        {
                            ...
                            int ccNumber = j + 1; // Для MIDI-сообщения используем индекс + 1
                            ...
                            // Обновляем параметр плагина, используя ccMapping активного банка:
                            if (auto* vstHost = getVSTHostComponent())
                            {
                                if (auto* bm = dynamic_cast<BankManager*>(bankTab.get()))
                                {
                                    ...
                                    if (activeBankIndex >= 0 && activeBankIndex < bm->getBanks().size())
                                    {
                                        int mappedCC = bm->getBanks()[activeBankIndex].ccMapping[j];
                                        vstHost->setPluginParameter(mappedCC, ccValue);
                                    }
                                    else
                                    {
                                        vstHost->setPluginParameter(j, ccValue);
                                    }
                                }
                                else
                                {
                        ...
                        for (int i = 0; i < presetButtons.size(); ++i)
                        {
                            ...
                            if (midiOut != nullptr)
                            {
                                auto msg = juce::MidiMessage::controllerEvent(1, presetCCNumbers[tempPresetIndex],
                        ...
                        if (auto* bm = dynamic_cast<BankManager*>(bankTab.get()))
                        {
                            ...
                            if (activeIndex >= 0 && activeIndex < bm->getBanks().size())
                            {
                                ...
                                int presetIndex = shiftButton->getToggleState() ? (3 + clickedIndex) : clickedIndex;
                                // Запоминаем новый текущий пресет (так же, как для CC состояния)
                                currentPresetIndex = presetIndex;
                                // Устанавливаем слайдер в соответствии с сохранённым значением для этого пресета
                                volumeSlider->setValue(bm->getBanks()[activeIndex].presetVolume[presetIndex],
