...
public:
    KeyboardWindow()
    {
        setWantsKeyboardFocus(true); // ëîâèì êëàâèøè
        setAlwaysOnTop(true);
...
private:
    ...
    void timerCallback() override
    {
        ...
        if (caretVisible)
        {
            ...
            // òåïåðü ïîêàçûâàåì êëàâèàòóðó ïðè ëþáîì äâèæåíèè èëè ïîÿâëåíèè êàðåòêè
            if (caretMoved)
            {
                suppressUntilCaretGone = false; // ñáðàñûâàåì áëîêèðîâêó
        ...
        else
        {
            ...
            if (isVisible()) hideKeyboard(false); // àâòîçàêðûòèå ïðè èñ÷åçíîâåíèè êàðåòêè
        }
    }
