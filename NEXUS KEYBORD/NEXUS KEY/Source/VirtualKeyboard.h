...
private:
    ...
    void addLetterKey(std::vector<juce::TextButton*>& row, juce_wchar c, float weight = 1.0f)
    {
        ...
        b->onClick = [this, b]() {
            // достаём базовый символ
            juce::String stored = b->getProperties()[juce::Identifier("baseChar")].toString();
            juce_wchar base = stored.isNotEmpty() ? stored[0] : L'?';

            // вычисляем регистр с учётом Shift и Caps
            bool upper = capsActive ^ shiftActive;
            ...
            // если Shift был одноразовый — сбрасываем
    ...
    void buildLettersLayout()
    {
        ...
        // Row 3: Shift + ZXCVBNM + Enter
        {
            ...
            shiftLeft->setColour(juce::TextButton::buttonColourId, keyColour);       // фон серый
            ...
            enterBtn->setColour(juce::TextButton::buttonColourId, keyColour);       // фон серый
            enterBtn->setColour(juce::TextButton::textColourOffId, juce::Colours::black); // текст чёрный
