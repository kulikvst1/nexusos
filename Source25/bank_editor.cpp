...
void BankEditor::showVSTDialog()
{
    ...
    // Ветка B: плагин загружен → спрашиваем о выгрузке.
    juce::AlertWindow::showOkCancelBox(
        ...
        "Yes",
        "Cancel",
        nullptr,  // ассоциированный компонент (можно оставить nullptr)
        juce::ModalCallbackFunction::create([this](int result)
            {
                // В типичной конфигурации result == 1 означает, что пользователь выбрал "Yes"
                // Но поскольку в плагинах бывают нюансы, рекомендуется вывести значение в лог:
                DBG("AlertWindow result: " << result);

                if (result == 1)
                {
                    // Выполняем выгрузку плагина и обновление интерфейса
                ...
                // Если пользователь выбрал "Cancel" (result != 1), ничего не делаем.
            })
    );
}
