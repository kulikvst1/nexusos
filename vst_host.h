...
private:
    void initialiseComponent()
    {
        ...
        // --- Создаём 6 кнопок для редактирования пресетов ---
        for (int i = 0; i < 6; ++i)
        {
            ...
            presetBtn->onClick = [this, i]()
                {
                    if (pluginInstance != nullptr)
                    {
                        // Создаём редактор пресета.
                        auto* presetEditor = new PresetEditorComponent(pluginInstance.get(), i);
                        // Устанавливаем callback, который будет вызван внутри редактора, когда пользователь нажмёт "Save Preset".
                        presetEditor->onPresetSaved = [this](int presetIndex, const std::map<int, float>& presetData)
                            {
                                // Сохраняем данные в in-memory контейнере для пресетов.
                                if (presetIndex < inMemoryPresets.size())
                                    inMemoryPresets[presetIndex] = presetData;
                                // Теперь, при необходимости, можно вызвать Alert или другое уведомление.
                                // (Но вы можете не выводить уведомление, если это не требуется.)
                                DBG("Preset " << (presetIndex + 1) << " saved via callback.");
                            };

                        // Открываем редактор заранее асинхронно, чтобы он оставался на экране,
                        // пока пользователь не нажмёт Save или Cancel.
                        juce::DialogWindow::LaunchOptions options;
                        options.dialogTitle = "EDIT PRESET SET " + juce::String(i + 1);
                        options.dialogBackgroundColour = juce::Colours::lightgrey;
                        options.resizable = true;
                        options.useNativeTitleBar = false;
                        options.content.setNonOwned(presetEditor);
                        // Задаём размеры редактора.
                        presetEditor->setSize(500, pluginInstance->getParameters().size() * 40 + 70);
                        options.launchAsync();
                    }
                    else
                    {
                        juce::AlertWindow::showMessageBoxAsync(juce::AlertWindow::InfoIcon,
                            "Info", "Plugin is not loaded!");
                    }
                };
    ...
    std::vector<std::map<int, float>> inMemoryPresets;
