...
public:
    ...
    // Метод, который обновляет BPM в playhead
    ...
    void loadPlugin(const juce::File& pluginFile, double sampleRate, int blockSize)
    {
        ...
        // Предполагается, что у вас уже создан pluginInstance и настроен currentSampleRate/currentBlockSize.
        pluginInstance->prepareToPlay(currentSampleRate, currentBlockSize);

        // После вызова prepareToPlay – создаём и настраиваем transport (CustomAudioPlayHead):
        double defaultBPM = 120.0; // Можно брать из настроек или slider'а
        customAudioPlayHead = std::make_unique<CustomAudioPlayHead>();
        customAudioPlayHead->setBpm(defaultBPM);
        customAudioPlayHead->setActive(true);  // Включаем транспорт
...
private:
    ...
    //juce::Label* tapTempoDisplay = nullptr;
