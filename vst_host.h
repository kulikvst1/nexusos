...
public:
    VSTHostComponent()
    {
        ...
        {
            // Убедитесь, что currentSampleRate и currentBlockSize уже установлены
            loadPlugin(lastPlugin, currentSampleRate, currentBlockSize);
        }
    }
    ...
    void buttonClicked(juce::Button* button) override
    {
        if (button == &loadButton)
        {
            if (selectedIndex >= 0 && selectedIndex < pluginFiles.size())
            {
                ...
                // Сохраняем путь выбранного плагина для автозагрузки в будущих запусках
        ...
        {
            unloadPlugin();
            // Сбрасываем сохранённый путь, чтобы при следующем запуске плагин не загружался автоматически
            clearSavedPluginFile();
            repaint();
        }
        else if (button == &parametersButton)
        {
            // Открываем окно редактирования параметров, если плагин загружен
            if (pluginInstance != nullptr)
            {
...
private:
    // Функция сохранения выбранного плагина в постоянное хранилище
    void saveSelectedPluginFile(const juce::File& file)
    {
        juce::PropertiesFile::Options options;
        options.applicationName = "NexusOS";   // Замените на имя вашего приложения
    ...
    // Функция загрузки пути к выбранному плагину из настроек
    ...
    // Новый метод: сброс сохранённого пути плагина
