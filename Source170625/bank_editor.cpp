...
void BankEditor::startLearn(int slot)
{
    ...
    btn.setColour(juce::TextButton::buttonColourId,
        juce::Colour(0, 200, 0));      // зелёный

    selectedPresetLabel.setText("Move any control on the plugin", juce::dontSendNotification);
...
void BankEditor::cancelLearn()
{
    ...
    /* ▼ СОБЫТИЕ НАРУЖУ */
    if (onLearnToggled) onLearnToggled(learnSlotIdx, false);
...
void BankEditor::toggleLearnFromHost(int cc, bool on)
{
    ...
    bool current = banks[activeBankIndex]
        .presetCCMappings[activePreset][cc].enabled;
    if (current == on) return;        // уже в нужном состоянии

    if (on)
        startLearn(cc);              // включает learn + флаги
    else
        cancelLearn();               // полностью погасить
}
