...
private:
    ...
    // атомарные гейны (вы уже их сделали)
    std::atomic<float> gainValL{ 1.0f }, gainValR{ 1.0f };

    // атомарные флаги для кнопок Solo/Mute/Link
    std::atomic<bool> atomicSoloL{ false };
    std::atomic<bool> atomicSoloR{ false };
    std::atomic<bool> atomicMuteL{ false };
    std::atomic<bool> atomicMuteR{ false };
    std::atomic<bool> atomicLink{ false };
