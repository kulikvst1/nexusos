...
private:
    ...
    //«DABLER» 
    Doubler         doubler;      
    std::atomic<float>   atomicDoublerDelayMs{ 7.0f };
    std::atomic<float> atomicMix{ 1.0f };
    std::atomic<bool>        doublerBypassed{ true };
    juce::TextButton    doublerButton{ "Dbl" };
    std::array<juce::Slider, 4> doublerSliders;
    std::array<juce::Label, 4> doublerNameLabels;
    juce::Label                doublerLabel;
