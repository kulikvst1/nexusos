...
public:
    ...
    // GUI-компонент финального мастера
    void setOutControl(OutControlComponent* oc) noexcept
    {
        outControl.store(oc, std::memory_order_release);
    }

    // Input (гейт, клипы, VU)
    void setInputControl(InputControlComponent* ic) noexcept
    {
        inputControl.store(ic, std::memory_order_release);
    }

private:
    // ВАЖНО: атомики для объектов, которые дергаются из аудиопотока
    std::atomic<InputControlComponent*>         inputControl{ nullptr };
    std::atomic<OutControlComponent*>           outControl{ nullptr };
    std::atomic<LooperEngine*>                  looper{ nullptr };

    std::atomic<juce::AudioPluginInstance*>     pluginInstance{ nullptr };

    // Остальные поля могут остаться неатомарными (используются в UI/главном потоке)
