...
public:
    ...
    // новый сеттер, если вам нужно заранее настроить тюнер по sampleRate и blockSize
    void setTunerParams(int blockSize, double sampleRate) noexcept
    {
        if (!tunerComponent)
        {
        ...
        tunerComponent->prepare(blockSize, sampleRate);
    }
private:
