...
public:
    ...
    void tap()
    {
        ...
        if (lastTapTime > 0.0)
        {
            double interval = now - lastTapTime; // мс между тапами

            // Защита от слишком длинной паузы (например, > 2 секунд)
            if (interval > 2000.0)
            {
                intervals.clear();   // сброс истории
                lastTapTime = now;   // начинаем новую серию
                return;              // BPM не трогаем
            }

            intervals.add(interval);

            // Ограничиваем число интервалов
            if (intervals.size() > maxIntervals)
                intervals.remove(0);

            // Обновляем BPM только если есть минимум два интервала
            if (intervals.size() >= 2)
            {
                double sum = 0.0;
