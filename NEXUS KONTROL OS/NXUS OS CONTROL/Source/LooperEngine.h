...
public:
    ...
    void LooperEngine::setMode(Mode newMode)
    {
        if (mode != newMode)
        {
            mode = newMode;

            // Если заходим в Looper и он пустой — явно ставим Clean
            if (mode == Mode::Looper && !hasData)
                state = Clean;

            // Гарантированно уведомляем подписчиков о смене режима
            if (onModeChanged)
                onModeChanged(mode);
    ...
    LooperEngine::Mode LooperEngine::getMode() const noexcept
    {
        return mode;
    }
