...
public:
        ...
        : tabs(TabbedButtonBar::TabsAtTop)
    {
        ...
        // ✅ ПРАВИЛЬНЫЙ ПОРЯДОК: сначала создаём движок, потом пробрасываем
        filePlayerEngine = std::make_unique<FilePlayerEngine>();
        filePlayerComponent = std::make_unique<FilePlayerComponent>(*filePlayerEngine);
        filePlayerEngine->prepareToPlay(512, 44100.0);
        vstHostComponent->setFilePlayerEngine(filePlayerEngine.get());
        ...
        tabs.addTab("PLAYER", Colour::fromRGBA(50, 62, 68, 255), filePlayerComponent.get(), false);
...
private:
    ...
    std::unique_ptr<FilePlayerEngine> filePlayerEngine;
    std::unique_ptr<FilePlayerComponent> filePlayerComponent;
