...
public:
        ...
        : tabs(TabbedButtonBar::TabsAtTop)
    {
        ...
        // 4) Создаём OutControlComponent и «подключаем» его в хост
        outControlComponent = std::make_unique<OutControlComponent>();
        vstHostComponent->setOutControlComponent(outControlComponent.get());

        // 4a) Создаём InControlComponent 
       // 4a) Создаём InputControlComponent 
        inputControlComponent = std::make_unique<InputControlComponent>();

        // 4b) Создаём SimpleGate и связываем его с UI
        gateProcessor = std::make_unique<SimpleGate>();
        gateProcessor->prepare(currentSampleRate);

        // Пробрасываем в InputControlComponent
        inputControlComponent->setGateProcessor(gateProcessor.get());

        // 4c) Регистрируем InputControlComponent в VSTHostComponent
    ...
    /** Возвращает AudioDeviceManager, инициализированный в этом компоненте */
...
private:
    ...
    std::unique_ptr<SimpleGate> gateProcessor;
