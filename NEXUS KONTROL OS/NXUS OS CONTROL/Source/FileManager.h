...
#if JUCE_WINDOWS
            // Если это исполняемый файл — запускаем напрямую; иначе открываем через ассоциации
            if (f.hasFileExtension("exe") || f.hasFileExtension("bat") || f.hasFileExtension("cmd"))
                f.startAsProcess();
            else
                juce::URL(f).launchInDefaultBrowser();
#elif JUCE_LINUX
            // На Linux: открыть через xdg-open (launchInDefaultBrowser)
            // Если файл действительно исполняемый (бит + доступ), startAsProcess тоже сработает
            juce::URL(f).launchInDefaultBrowser();
#elif JUCE_MAC
            // На macOS: открыть через 'open' (launchInDefaultBrowser)
            juce::URL(f).launchInDefaultBrowser();
#endif
        }
    }
