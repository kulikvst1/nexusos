...
void BankEditor::buttonClicked(juce::Button* b)
{
    ...
    else if (b == &loadButton)
    {
        ...
        fileChooser->launchAsync(
            ...
            [this](const juce::FileChooser& fc)
            {
                ...
                if (auto* bankEl = xml->getChildByName("Bank"))
                {
                    // Загружаем в активный банк
                    deserializeBank(banks[activeBankIndex], *bankEl);

                    // Применяем к плагину
                    applyBankToPlugin(activeBankIndex);
     ...
     else if (b == &saveButton)
    {
        fileChooser = std::make_unique<juce::FileChooser>(
            "Save bank as...",
            juce::File::getSpecialLocation(juce::File::userDocumentsDirectory)
            .getChildFile(banks[activeBankIndex].bankName + ".xml"),
            "*.xml"
        );
        ...
        fileChooser->launchAsync(
            ...
            {
                auto file = fc.getResult();
                if (file == juce::File()) // отмена
                    return;

                // Обновляем baseline/diffs для активного банка
                snapshotCurrentBank();

                // Создаём XML только с одним банком
                juce::XmlElement root("BanksConfig");
                root.addChildElement(serializeBank(banks[activeBankIndex], activeBankIndex));
...
{
    if (newIdx == activeBankIndex)
        return;

    snapshotCurrentBank(); // сохраняем текущее состояние

    activeBankIndex = juce::jlimit(0, numBanks - 1, newIdx);

    applyBankToPlugin(activeBankIndex); // применяем новый банк
...
void BankEditor::loadSettingsFromFile(const juce::File& file)
{
    ...
    bankSnapshot = banks[activeBankIndex]; // фиксируем эталон
    updateUI();
    applyBankToPlugin(activeBankIndex);
}
...
void BankEditor::applyBankToPlugin(int bankIndex)
{
    if (bankIndex < 0 || bankIndex >= (int)banks.size())
        return;
    if (!vstHost)
    ...
    // --- Определяем, нужно ли грузить плагин
    if (!currentInst)
    {
        // Плагин не загружен — грузим, если есть путь
        needLoad = b.pluginId.isNotEmpty();
    }
    else
    {
        // Плагин загружен, но другой — выгружаем
        if (b.pluginId.isNotEmpty() && b.pluginId != lastLoadedPluginId)
        {
            vstHost->unloadPlugin();
            needLoad = true;
        }
    ...
    // --- Загружаем новый плагин, если нужно
    if (needLoad)
    {
    ...
    // --- 1. Если есть полный state — применяем только его
    if (b.pluginState.getSize() > 0)
    {
        auto stateCopy = b.pluginState;
        juce::MessageManager::callAsync([inst, stateCopy]()
            {
                if (inst != nullptr)
                {
                    inst->setStateInformation(stateCopy.getData(),
                        (int)stateCopy.getSize());
                    DBG("Plugin state applied asynchronously in message thread");
                }
            });
        return; // baseline/diffs не трогаем
    }

    // --- 2. Если state пуст — baseline + diffs
    if (b.activeProgram >= 0)
        inst->setCurrentProgram(b.activeProgram);
