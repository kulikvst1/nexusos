...
void BankEditor::buttonClicked(juce::Button* b)
{
        ...
        fileChooser->launchAsync(
            ...
            [this](const juce::FileChooser& fc)
            {
                ...
                if (!file.existsAsFile()) // отмена
                    return;
                // Загружаем весь BanksConfig, как при автосейве
                loadSettingsFromFile(file);
                // Обновляем UI
                updateUI();
                // Применяем активный банк к плагину
                applyBankToPlugin(activeBankIndex, true); // если сделал универсальный метод с флагом
                fileChooser.reset();
            }
        );
    }
    ...
    else if (b == &saveButton)
    {
        fileChooser = std::make_unique<juce::FileChooser>(
            "Save project as...",
            juce::File::getSpecialLocation(juce::File::userDocumentsDirectory)
            .getChildFile("MyPluginBanks.xml"),
            "*.xml"
        );
        ...
        fileChooser->launchAsync(
            ...
            [this](const juce::FileChooser& fc)
            {
                ...
                if (file == juce::File()) // отмена
                    return;
                // Делаем всё как при Store
                snapshotCurrentBank();      // фиксируем baseline/diffs
                allowSave = true;
                storeToBank();              // обновляем дефолтный конфиг
                allowSave = false;
                // Теперь сохраняем весь BanksConfig в выбранный файл
