...
// Learn SW1
    pedalLearn1.setButtonText("Learn SW1");
    // цвет как у обычных Learn
    pedalLearn1.setColour(juce::TextButton::buttonColourId,
        learnButtons[0].findColour(juce::TextButton::buttonColourId));
    addAndMakeVisible(pedalLearn1);

    // Learn SW2
    pedalLearn2.setButtonText("Learn SW2");
    pedalLearn2.setColour(juce::TextButton::buttonColourId,
        learnButtons[0].findColour(juce::TextButton::buttonColourId));
    addAndMakeVisible(pedalLearn2);

    // Set Pedal 1
    pedalSlot1Button.setButtonText("Set Pedal 1");
    // цвет как у обычных Set CC
    pedalSlot1Button.setColour(juce::TextButton::buttonColourId,
        setCCButtons[0].findColour(juce::TextButton::buttonColourId));
    addAndMakeVisible(pedalSlot1Button);

    // Set Pedal 2
    pedalSlot2Button.setButtonText("Set Pedal 2");
    pedalSlot2Button.setColour(juce::TextButton::buttonColourId,
        setCCButtons[0].findColour(juce::TextButton::buttonColourId));
    addAndMakeVisible(pedalSlot2Button);

    // ─────────────── Callbacks ───────────────

    // пока простая заглушка: открываем SetCCDialog
    pedalSlot1Button.onClick = [this]() {
        auto dlg = new SetCCDialog(vstHost, pedalMapping1, "SW1 (FOOT CONTROL)",
            [this](CCMapping m, bool ok) {
                if (ok) pedalMapping1 = m;
            });
        };

    pedalSlot2Button.onClick = [this]() {
        auto dlg = new SetCCDialog(vstHost, pedalMapping2, "SW2 (FOOT CONTROL)",
            [this](CCMapping m, bool ok) {
                if (ok) pedalMapping2 = m;
            });
        };

    // Learn‑кнопки пока можно оставить пустыми или включить подсветку режима
    pedalLearn1.onClick = [this]() {
        // TODO: включить режим Learn для SW1
        };
    pedalLearn2.onClick = [this]() {
        // TODO: включить режим Learn для SW2
        };
...
void BankEditor::resized()
{
    ...
    // —————————— Row 5: SET CC buttons ——————————
    for (int i = 0; i < numCCParams; ++i)
        setCCButtons[i].setBounds(baseX + i * 2 * sW, baseY + 5 * sH, 2 * sW, sH);

    // —————————— Row 6: CC name editors ——————————
    for (int i = 0; i < numCCParams; ++i)
        ccNameEditors[i].setBounds(baseX + i * 2 * sW, baseY + 6 * sH, 2 * sW, sH);

    // —————————— Row 7: CC toggle buttons ——————————
    for (int i = 0; i < numCCParams; ++i)
        ccToggleButtons[i].setBounds(baseX + i * 2 * sW, baseY + 7 * sH, 2 * sW, sH);

    // —————————— Row 8: Pedal Learn buttons ——————————
    pedalLearn1.setBounds(baseX + 0 * 2 * sW, baseY + 8 * sH, 2 * sW, sH);
    pedalLearn2.setBounds(baseX + 1 * 2 * sW, baseY + 8 * sH, 2 * sW, sH);

    // —————————— Row 9: Pedal Set buttons ——————————
    pedalSlot1Button.setBounds(baseX + 0 * 2 * sW, baseY + 9 * sH, 2 * sW, sH);
    pedalSlot2Button.setBounds(baseX + 1 * 2 * sW, baseY + 9 * sH, 2 * sW, sH);
