...
void BankEditor::buttonClicked(juce::Button* b)
{
    ...
    {
        auto bankDir = juce::File::getSpecialLocation(juce::File::userApplicationDataDirectory)
        ...
        fm->setConfirmCallback([this](const juce::File& file)
            {
                ...
                updateUI();
                // applyBankToPlugin уже вызовется внутри loadSettingsFromFile через Timer
            });
    ...
    else if (b == &storeButton)
    {
        auto doStore = [this]()
            {
                allowSave = true;
                storeToBank(); // storeToBank не трогает pluginId
                allowSave = false;
    ...
    {
        auto saveDir = juce::File::getSpecialLocation(juce::File::userApplicationDataDirectory)
            .getChildFile("NEXUS_KONTROL_OS")
            .getChildFile("Banks");
...
void BankEditor::loadSettingsFromFile(const juce::File& file)
{
    ...
    globalPluginId = normalizePluginId(xml->getStringAttribute("pluginId")); // <-- нормализация
    ...
    forEachXmlChildElementWithTagName(*xml, bankEl, "Bank")
    {
        ...
        b.pluginId = normalizePluginId(bankEl->getStringAttribute("pluginId")); // <-- нормализация
...
void BankEditor::saveSettingsToFile(const juce::File& file)
{
    ...
    for (int i = 0; i < static_cast<int>(banks.size()); ++i)
    {
        ...
        bankEl->setAttribute("pluginId", normalizePluginId(b.pluginId)); // <-- исправлено
    ...
    if (vstHost->getPluginInstance() == nullptr)
    {
        ...
        menu.showMenuAsync(
            ...
            [this, entries, indexMap](int choice)
            {
                ...
                const auto& desc = entries[realIdx].desc;  // исправлено
