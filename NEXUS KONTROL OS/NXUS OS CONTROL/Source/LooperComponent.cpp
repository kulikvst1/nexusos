...
void LooperComponent::resized()
{
    ...
    {
        auto sMode = span(1, 1, 3).reduced(mx, 0);
        int hMode = sector(1, 2).getHeight() / 2;
        int yMode = sMode.getY() + sector(1, 2).getHeight() / 2;
        modeLabel.setBounds(sMode.getX(), yMode, sMode.getWidth(), hMode);
    }

    {
        auto sProgress = span(1, 1, 3);
        int hProg = sProgress.getHeight() / 4;
        int yProg = sProgress.getY(); // можно опустить ниже, если нужно
        progressSlider.setBounds(sProgress.getX(), yProg, sProgress.getWidth(), hProg);
    }

    // === ROW 2 ===
    fileSelectBtn.setBounds(sector(2, 0));
    resetBtn.setBounds(sector(2, 1));
    controlBtn.setBounds(sector(2, 2));
    triggerBtn.setBounds(sector(2, 3));
    modeSwitchBtn.setBounds(sector(2, 4));
}
...
void LooperComponent::buttonClicked(juce::Button* b)
{
    ...
    // === Обработка fileSelectBtn вне зависимости от режима ===
    if (b == &fileSelectBtn)
    {
        DBG("File select button clicked");
        ...
        fileChooser->launchAsync(juce::FileBrowserComponent::openMode | juce::FileBrowserComponent::canSelectFiles,
            [this](const juce::FileChooser& fc)
            {
                ...
                if (file.existsAsFile())
                {
                    ...
                    DBG("File load result: " + juce::String(success ? "success" : "fail"));
...
void LooperComponent::updateStateLabel()
{
    if (engine.getMode() == LooperEngine::Mode::Looper)
    {
       ...
       // modeLabel.setColour(juce::Label::backgroundColourId, juce::Colours::white);
       // modeLabel.setColour(juce::Label::textColourId, juce::Colours::black);
        fileSelectBtn.setVisible(false);
    }
    else
    {
