    ...
    : engine(eng)
{
 ...
 //   for (auto* sl : { &levelSlider, &mixSlider, &progressSlider })
  //      sl->setLookAndFeel(&buttonLnf);
    ...
    levelSlider.setColour(juce::Slider::rotarySliderFillColourId, juce::Colours::red);  // зона заполнения
    levelSlider.setColour(juce::Slider::rotarySliderOutlineColourId, juce::Colours::black);       // обводка
