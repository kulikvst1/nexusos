...
void Rig_control::setOutControlComponent(OutControlComponent* oc) noexcept
{
    ...
    {
        // 1) Подписываемся на апдейт «мастер-гейна» из OutControl
        outControl->onMasterGainChanged = [this](float newAvgDb)
            {
                // конвертируем дБ → raw 0…127
                int raw = (int)juce::jmap<float>(newAvgDb,
                    -60.0f, 12.0f,   // входной дБ-диапазон
                    0.0f, 127.0f);  // выходной raw

                // ставим свой слайдер без уведомления listener’ов
                volumeSlider->setValue(raw, juce::dontSendNotification);

                // сохраняем как предыдущее значение для расчёта дельты
                prevVolDb = newAvgDb;
            };

        // 2) Синхронизируем начальное положение Rig_control с текущим OutControl
...
void Rig_control::sliderValueChanged(juce::Slider* slider)
{
    ...
    {
        // 1) абсолютное новое значение в дБ
        float newVolDb = juce::jmap<float>(
            (float)volumeSlider->getValue(),  // raw
            0.0f, 127.0f,                    // raw range
            -60.0f, 12.0f                     // dB range
        );

        // 2) вычисляем сдвиг относительно предыдущего
        float deltaDb = newVolDb - prevVolDb;
        prevVolDb = newVolDb;

        // 3) сдвигаем оба канала в OutControlComponent
        if (outControl != nullptr)
            outControl->offsetGainDb(deltaDb);

        return; // больше ничего не делаем в этом вызове
    }
