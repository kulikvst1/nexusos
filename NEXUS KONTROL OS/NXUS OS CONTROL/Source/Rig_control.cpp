...
void Rig_control::resized()
{
    ...
    // высота кнопочного ряда = половина высоты сектора
    int btnH = cell26.getHeight() / 2;

    // строим прямоугольник, привязанный к нижней половине
    auto btnRow = juce::Rectangle<int>(
        cell26.getX(),                       // X того же левого края
        cell26.getBottom() - btnH,           // Y = низ сектора минус btnH
        cell26.getWidth(),                   // полная ширина сектора
        btnH                                  // высота = btnH
    ).reduced(m2);                           // «отступаем» margin по всем сторонам

    // теперь btnRow уже с учётом margin — можно делить на две кнопки
    ...
    // 4) гарантируем, что кнопки всегда поверх лупера/тюнера
    looperBtn.toFront(false);
    tunerBtn.toFront(false);

    // 5) если движок не сконфигурирован — прячем лупер и выходим
    ...
    // 6) лениво создаём компоненты (без повторного prepare)
    if (!looperComponent)
    {
    ...
    // 7) общая зона под оба компонента
    ...
    // 8) показываем / прячем компоненты
    looperComponent->setBounds(
        looperComponent->isVisible() ? sharedArea
        : juce::Rectangle<int>(0, 0, 0, 0));
    tunerComponent->setBounds(
        tunerComponent->isVisible() ? sharedArea
        : juce::Rectangle<int>(0, 0, 0, 0));
...
void Rig_control::buttonClicked(juce::Button* button)
{
    ...
    if (button == &tunerBtn)
    {
        // лениво создаём компонент, если ещё не создали
        if (!tunerComponent)
        {
            tunerComponent = std::make_unique<TunerComponent>();
            addAndMakeVisible(*tunerComponent);
            // prepare(...) у вас уже вызывался где нужно
        }

        // переключаем видимость
        tunerComponent->setVisible(tunerBtn.getToggleState());
        // сразу пересчитаем layout
        resized();
        return;
    }
