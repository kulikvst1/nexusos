...
void Rig_control::resized()
{
    ...
    // ─── PEDAL MODE LABEL ───
    {
        auto sector0 = getSectorRect(1).reduced(1); // нумерация с 1!
        int w = sector0.getWidth();
        int h = sector0.getHeight() / 3;

        // прямоугольник сверху сектора
        juce::Rectangle<int> modeBounds(sector0.getX(), sector0.getY(), w, h);

        pedalModeLabel.setBounds(modeBounds);
        pedalModeLabel.setJustificationType(juce::Justification::centred);
        pedalModeLabel.setOpaque(false);
        // шрифт пропорционален высоте
        pedalModeLabel.setFont(juce::Font((float)h * 0.6f, juce::Font::bold));
    }
    // ─── INPUT CLIP ───
{
    auto inputSector = getSectorRect(1).reduced(1); // сектор №0 → индекс 1 в массиве
    int gap = 4;
    int halfW = (inputSector.getWidth() - gap) / 2;
    int clipH = inputSector.getHeight() / 6;
    int yClip = inputSector.getBottom() - clipH;
    ...
    int gap = 4; // пикселей между ними
    int halfW = (volumeSector.getWidth() - gap) / 2;

    // делаем их пониже: 1/6 сектора
    int clipH = volumeSector.getHeight() / 6;

    // размещаем в нижней части сектора
    int yClip = volumeSector.getBottom() - clipH;
