        ...
        juce::MessageManager::callAsync([this, cc, value, isOn]()
            {
                ...
                case 1: // ON/OFF STATUS
                    if (value == 0) {
                        ...
                        bool invertState = inputControl->getInvertState();
                ...
                case 2: // SW1
                    if (pedalOn)
                    {
                        ...
                        if (bankEditor)
                            bankEditor->updateCCParameter(12, isOn ? 127 : 0); // слот 12 = SW1
                    }
                    break;
                ...
                case 3: // SW2
                    if (pedalOn)
                    {
                        inputControl->syncSwitchState(cc, isOn);
                        if (bankEditor)
                            bankEditor->updateCCParameter(13, isOn ? 127 : 0); // слот 13 = SW2
                    }
                    break;
                case 4: // Pedal value
                    if (pedalOn)
                    {
                        ...
                        // пробрасываем в BankEditor
                        if (bankEditor)
                        {
                            if (currentPedalMode == PedalMode::SW1)
                                bankEditor->applyPedalValue(10, norm); // слот 10 = SW1
                            else if (currentPedalMode == PedalMode::SW2)
                                bankEditor->applyPedalValue(11, norm); // слот 11 = SW2
                        }
                    }
                    break;
