...
void Rig_control::resized()
{
    ...
    // Предвычисляем прямоугольники секторов
    ...
    // ─── СЕКТОР 20: up, down, stomp ───
    {
        auto sec20 = getSectorRect(20).reduced(4);
    ...
    // ─── СЕКТОР 26: looper, tuner, shift, tempo ───
    {
        auto sec26 = getSectorRect(26).reduced(4);
    ...
    //switch
    if (sButtons[0])
        sButtons[0]->setBounds(getSectorRect(10).reduced(4)); // S1

    if (sButtons[1])
        sButtons[1]->setBounds(getSectorRect(19).reduced(4)); // S2

    if (sButtons[2])
        sButtons[2]->setBounds(getSectorRect(18).reduced(4)); // S3

    if (sButtons[3])
        sButtons[3]->setBounds(getSectorRect(27).reduced(4)); // S4
    // После расстановки — обновляем их состояние
    updateAllSButtons();
    ...
    bankEditor->onBankChanged = [this]()
        {
            ...
            // 🔹 Сбрасываем STOMP в дефолт
            stompMode = false;
            stompBtn.setToggleState(false, juce::dontSendNotification);

            // Возвращаем preset-кнопки в пресетный режим
            for (auto* btn : presetButtons)
                btn->setRadioGroupId(100, juce::dontSendNotification);

            updateAllSButtons(); // чтобы S2 вернулся в дефолт

            // После сброса всегда отрисовываем пресетный режим
            updatePresetDisplays();
        };

    // 3) любое редактирование текста
...
void Rig_control::updateStompDisplays()
{
    ...
    for (int i = 0; i < 3; ++i)
    {
        // Индексы CC для кнопки и метки
        int btnCC = shiftOn ? (i + 3) : i;   // CC1..CC3 или CC4..CC6
        int lblCC = shiftOn ? i : (i + 3);   // обратная тройка
