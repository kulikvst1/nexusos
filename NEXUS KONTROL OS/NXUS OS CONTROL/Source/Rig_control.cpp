...
void Rig_control::setBankEditor(BankEditor* editor)
{
    ...
    // подписки на события BankEditor
...
{
    if (!bankEditor || stompMode || (libraryLogic && libraryLogic->isActive()))
        return;
...
void Rig_control::buttonClicked(juce::Button* button)
{
    ...
    if (button->getRadioGroupId() == 100)
    {
        ...
        if (idx >= 0)
        {
            // ✅ грузим только если кнопка включена
            if (btn->getToggleState())
            {
                ...
                if (libraryLogic && libraryLogic->isActive())
                {
                    // В библиотечном режиме → грузим файл
                    selectLibraryFile(idx);
                }
                else
                {
                    // В обычном режиме → пресеты
    ...
    if (button == &looperBtn && enginePtr != nullptr)
    {
        ...
        if (onLooperButtonChanged)
            onLooperButtonChanged(looperBtn.getToggleState());
...
void Rig_control::updateAllSButtons()
{
    ...
    if (shiftButton)
    {
        ...
        {
            // Library только если лупер не активен
            updateSButton(3, "NEXT", juce::Colours::green);
        }
        else if (shiftButton->getToggleState())
        {
...
void Rig_control::setTunerState(bool on)
{
    ...
    if (midiOut)
    {
        int value = on ? 127 : 0;
        midiOut->sendMessageNow(
            juce::MidiMessage::controllerEvent(1, 110, value) // ch1 cc102
        );
    }
}
