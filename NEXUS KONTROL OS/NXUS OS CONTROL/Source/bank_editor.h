...
public:
    ...
    struct Bank
    {
        ...
        {
            // Имя банка — без учёта регистра и пробелов
            ...
            // Игнорируем pluginState и paramDiffs — они могут меняться сами
            return true;
        }
...
private:
   ...
   // bool             isSettingPreset = false;   // переносим сюда (раньше был глобал)
    ...
    // ─────────────── Pedal slots ───────────────
    CCMapping pedalMapping1;
    CCMapping pedalMapping2;

    juce::TextButton pedalLearn1{ "Learn SW1" };
    juce::TextButton pedalLearn2{ "Learn SW2" };

    juce::TextButton pedalSlot1Button{ "Set Pedal 1" };
    juce::TextButton pedalSlot2Button{ "Set Pedal 2" };
