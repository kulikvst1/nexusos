...
public:
    ...
    float getMeterLevelL() const noexcept { return meterLevelL.load(std::memory_order_acquire); }
    float getMeterLevelR() const noexcept { return meterLevelR.load(std::memory_order_acquire); }
...
private:
    ...
    std::atomic<float> meterLevelR{ 0.0f };
