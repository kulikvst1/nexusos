...
// Gate BYPASS
    for (auto* b : { &gateBypass1, &gateBypass2 })
    {
        ...
        updateBypassButtonColour(*b); // начальная синхронизация
    }
    // Input BYPASS
...
void InputControlComponent::resized()
{
    ...
    // VU
    {
        ...
        auto scaleWidth = meterWidth;

        juce::Rectangle<int> meterArea = juce::Rectangle<int>(meterWidth, col.getHeight())
            .withCentre(col.getCentre());

        juce::Rectangle<int> scaleArea = meterArea.translated(scaleWidth + 4, 0)
            .withWidth(scaleWidth); // ← зеркально справа

        vuLeft.setBounds(meterArea);
        vuScaleL.setBounds(scaleArea);
    }
    {
        ...
        juce::Rectangle<int> scaleArea = meterArea.translated(-scaleWidth - 4, 0)
            .withWidth(scaleWidth); // ← остаётся слева

        vuRight.setBounds(meterArea);
        vuScaleR.setBounds(scaleArea);
    }
    ...
    // Gate bypass
    gateBypass1.setBounds(ctrBox(cell(1), cell(1).getWidth() / 4, cell(1).getHeight() / 4));
    gateBypass2.setBounds(ctrBox(cell(4), cell(4).getWidth() / 4, cell(4).getHeight() / 4));
    inputBypass1.setBounds(ctrBox(cell(0), cell(4).getWidth() / 4, cell(4).getHeight() / 4));
    inputBypass2.setBounds(ctrBox(cell(5), cell(4).getWidth() / 4, cell(4).getHeight() / 4));
    // Верхние метки входа
    ...
    // ==== PEAK метки над метрами ====
    {
        ...
        const int gap = 4; // зазор между меткой и верхом метра
