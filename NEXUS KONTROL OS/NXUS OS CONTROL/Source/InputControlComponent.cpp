...
// // Gate BYPASS
    ...
    // Колбэк
    invertButton.onStateChange = [this]() {
        ...
        if (rigControl && isPedalConnected) {
            rigControl->sendMidiCC(6, 10, on ? 127 : 0);
        }
        };
    loadSettings();
}
...
// 0-я строка: только заголовок
...

    {
        ...
        // Слайдер STATE — на одном уровне по низу с MIN/MAX, удлинён вверх с учётом pedalTopOffset
        {
            auto s = pedalMinSlider.getBounds();
            int centerX = pedalStateTextLabel.getBounds().getCentreX();
            int newY = cell(6).getY() + pedalTopOffset;
            int newH = s.getBottom() - newY;
            pedalStateSlider.setBounds(centerX - s.getWidth() / 2, newY, s.getWidth(), newH);
        }
    }
    ...
    float autoBtnWidthFactor = 1.2f;   // ← множитель ширины кнопки автокалибровки относительно SW

    // Кнопки AutoConfig и Invert — на местах SW1 и SW2
    {
...
void InputControlComponent::syncPedalSliderByCC(int cc, int value)
{
    ...
    case 4: // Pedal State
        ...
        if (rigControl)
       //     rigControl->sendMidiCC(6, 4, value); // отправляем наружу
        break;
...
void InputControlComponent::setPedalConnected(bool connected)
{
    ...
    setButtonDimmed(invertButton, !connected);   // <--- добавляем INVERT
