...
void InputControlComponent::resized()
{
    ...
    // Блок импеданса
    {
        const int buttonSize = S(50);
        const int spacing = S(20, true);
        const int verticalOffset = S(150, true);
        ...
        int totalHeight = (buttonSize * 4) + (spacing * 3);
        int x = meterArea.getRight() + S(20);
        int y = ((getHeight() - totalHeight) / 2) + verticalOffset;
...
// 0-я строка: только заголовок
    ...
    // 2-я строка: PEDAL VALUE — по центру между колонками 0 и 1
    {
        auto c0 = cell(12); // row=2, col=0
        auto c1 = cell(13); // row=2, col=1

        // Объединяем две ячейки и центрируем
        juce::Rectangle<int> combined = c0.withRight(c1.getRight());

        // Центрируем rotary и метку
        placeTriple(pedalStateSlider, pedalStateTextLabel, combined, false, 0);
    }


    // 3-я строка: MIN и MAX — опущены вниз
    int pedalBottomOffset = S(30, true); // ← регулируй глубину посадки

    {
        auto cMin = cell(18).translated(0, pedalBottomOffset); // row=3, col=0
        auto cMax = cell(19).translated(0, pedalBottomOffset); // row=3, col=1

        placeTriple(pedalMinSlider, pedalMinTextLabel, cMin, false, 0);
        placeTriple(pedalMaxSlider, pedalMaxTextLabel, cMax, false, 0);
    }
    // Кнопка автокалибровки — по центру между MIN и MAX, поднята выше
    {
        auto cMin = cell(18);
        auto cMax = cell(19);
        auto combined = cMin.withRight(cMax.getRight());

        const int btnW = combined.getWidth() / 5;
        const int btnH = combined.getHeight() / 4;

        autoConfigButton.setBounds(juce::Rectangle<int>(btnW, btnH)
            .withCentre(combined.removeFromTop(S(-20, true)).getCentre())); // ← поднята выше
    }
    // Кнопки SW1 и SW2 — по центру строки 2, выровнаны по Y с pedalStateSlider
    {
        const int btnW = cell(12).getWidth() / 4;
        const int btnH = cell(12).getHeight() / 4;

        // Получаем центр rotary-слайдера
        int y = pedalStateSlider.getBounds().getCentreY() - (btnH / 2);

        sw1Button.setBounds(cell(12).getCentreX() - btnW / 2, y, btnW, btnH);
        sw2Button.setBounds(cell(13).getCentreX() - btnW / 2, y, btnW, btnH);
    }
...
{
    if (cc == 2) {
        sw1Button.setToggleState(isOn, juce::dontSendNotification);
        updateSwitchButtonColour(sw1Button, isOn);

        if (isOn)
            updateStatusLabel("SWITCH 1");
        else if (!sw2Button.getToggleState())
            stateLabel.setText("", juce::dontSendNotification);
    }

    if (cc == 3) {
        sw2Button.setToggleState(isOn, juce::dontSendNotification);
        updateSwitchButtonColour(sw2Button, isOn);

        if (isOn)
            updateStatusLabel("SWITCH 2");
        else if (!sw1Button.getToggleState())
            stateLabel.setText("", juce::dontSendNotification);
    }
}
void InputControlComponent::syncPedalSliderByCC(int cc, int value)
{
    ...
    switch (cc)
    {
    case 4: pedalStateSlider.setValue(value, juce::dontSendNotification); break;
    case 5: pedalMinSlider.setValue(value, juce::dontSendNotification); break;
    case 6: pedalMaxSlider.setValue(value, juce::dontSendNotification); break;
    default: break;
    }
}
