    ...
    currentBlockSize(512)
{
    ...
    for (int i = 0; i < 10; ++i)
    {
        ...
        ccButtons.add(btn);
        addChildComponent(btn); // добавляем, но не делаем видимым
        btn->setVisible(false); // остаётся скрытым
...
void VSTHostComponent::resized()
{
    ...
    // CC Buttons — точно такой же размер, как Learn‑кнопки, но выше
    static constexpr int kNumLearn = 10;
    for (int i = 0; i < kNumLearn; ++i)
    {
        auto learnBounds = learnBtn[i].getBounds();

        // копируем ширину и высоту Learn‑кнопки
