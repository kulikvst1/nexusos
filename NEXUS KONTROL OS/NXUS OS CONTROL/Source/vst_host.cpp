    ...
    currentBlockSize(512)
{
    ...
    for (int i = 0; i < 10; ++i)
    {
        ...
        lbl->setText("<none>", juce::dontSendNotification);
        lbl->setJustificationType(juce::Justification::centred);
        lbl->setFont(juce::Font(14.0f));

        // 🔹 Синий фон, белый текст
        lbl->setColour(juce::Label::backgroundColourId, juce::Colours::blue);
        lbl->setColour(juce::Label::textColourId, juce::Colours::white);

        // 🔹 Нередактируемая
        lbl->setInterceptsMouseClicks(false, false);

        // 🔹 Скрыта по умолчанию (нет плагина)
...
void VSTHostComponent::resized()
{
    ...
    // CC Labels строго над каждой Learn‑кнопкой
    static constexpr int kNumLearn = 10;
    for (int i = 0; i < kNumLearn; ++i)
    {
        auto btnBounds = learnBtn[i].getBounds();

        // метка такой же ширины, чуть выше кнопки
        auto labelBounds = btnBounds.withHeight(20)
            .withY(btnBounds.getY() - 22);

        // небольшой зазор по бокам
        labelBounds.reduce(2, 0);

        ccLabels[i]->setBounds(labelBounds);
    }
...
{
    if (slot >= 0 && slot < ccLabels.size())
        ccLabels[slot]->setText(text.isNotEmpty() ? text : "<none>", juce::dontSendNotification);
}
