...
private:
    bool isFullScreenMode{ false }, isLooperShown{ true },
        isTunerShown{ true }, isOutMasterShown{ true },
        boundsRestored{ false };
    ...
    void loadAudioSettings(MainContentComponent& content)
    {
        ...
        DBG("[LOAD AUDIO] from " << file.getFullPathName());

        if (!file.existsAsFile())
        {
            DBG("[LOAD AUDIO] no file – using defaults");
    ...
    void saveAudioSettings()
    {
        if (auto* mcc = dynamic_cast<MainContentComponent*> (getContentComponent()))
        {
            if (auto xml = mcc->getAudioDeviceManager().createStateXml())
            {
                ...
                DBG("[SAVE AUDIO] to " << file.getFullPathName());
            }
        }
    }
    ...
    void loadWindowState()
    {
        ...
        DBG("[LOAD WINDOW] from " << file.getFullPathName());
    ...
    void saveWindowState()
    {
        ...
        DBG("[SAVE WINDOW] to " << file.getFullPathName());
    }
