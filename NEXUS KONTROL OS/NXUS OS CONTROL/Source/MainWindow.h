...
public:
    enum MenuIds
    {
        ...
        miPluginManager = 3,
        miFullScreen = 4,
        miToggleLooper = 5,  // уже было
        miToggleTuner = 6   // ← добавили пункт для тюнера
    };
    ...
    MainWindow(const String& title)
        ...
        isLooperShown(true),
        isTunerShown(true)    // ← состояние тюнера по умолчанию
    {
        loadAudioSettings();
        loadWindowState();      // ← теперь читает оба флага
        ...
        // применяем сохранённые состояния видимости вкладок
        content->setLooperTabVisible(isLooperShown);
        content->setTunerTabVisible(isTunerShown);  // ← тюнер
    ...
    ~MainWindow() override
    {
        ...
        saveWindowState();      // ← сохраняет оба флага
    }
    ...
    PopupMenu getMenuForIndex(int, const String&) override
    {
        ...
        m.addSeparator();
        m.addItem(miFullScreen,
            "Full Screen", true, isFullScreenMode);
        m.addSeparator();
        m.addItem(miToggleLooper,
            "Show LOOPER TAB", true, isLooperShown);
        m.addItem(miToggleTuner,
            "Show TUNER TAB", true, isTunerShown);  // ← тюнер
        m.addSeparator();
...
private:
    ...
    bool isTunerShown{ true };   // ← добавленный флаг
    ...
    void saveWindowState()
    {
        ...
        xml.setAttribute("showTunerTab", isTunerShown);   // ← сохраняем тюнер
    ...
    void loadWindowState()
    {
        ...
        if (f.existsAsFile())
        {
            if (auto xml = XmlDocument::parse(f))
            {
                ...
                isTunerShown = xml->getBoolAttribute("showTunerTab", true);  // ← читаем тюнер
