...
OutControlComponent::OutControlComponent()
{
 ...
 // =========================================================================
    ...
    doublerSliders[0].setRange(10.0, 200.0, 1.0);
...
void OutControlComponent::processAudioBlock(juce::AudioBuffer<float>& buffer) noexcept
{
    ...
    if (++dspUpdateCounter >= dspUpdateInterval)//иногда сдесь остановка кода
    {
        dspUpdateCounter = 0;
    ...
    /*
    doubler.setDelayMs(atomicDoublerDelayMs.load(std::memory_order_relaxed));
    doubler.setMix(atomicMix.load(std::memory_order_relaxed));
    doubler.setBypass(doublerBypassed.load(std::memory_order_relaxed));
   if (!doublerBypassed.load(std::memory_order_relaxed))
...
void OutControlComponent::saveSettings() const
{
    ...
    /*
    // 2.1) сохраняем настройки даблера
    xml->setAttribute("doublerDelayMs",
        atomicDoublerDelayMs.load(std::memory_order_relaxed));
    xml->setAttribute("doublerBypassed",
        doublerBypassed.load(std::memory_order_relaxed));
    xml->setAttribute("doublerMix",
        atomicMix.load(std::memory_order_relaxed));
        */
    // 3) пишем в файл
...
void OutControlComponent::loadSettings()
{
    ...
    gainSliderR.setValue(xml->getDoubleAttribute("gainR", gainSliderR.getValue()),
        juce::sendNotification);
    /*
    // 2.1) загружаем параметры даблера
    // delay
    auto dblDelay = static_cast<float>(
        xml->getDoubleAttribute("doublerDelayMs",
            atomicDoublerDelayMs.load(std::memory_order_relaxed)));
    atomicDoublerDelayMs.store(dblDelay, std::memory_order_relaxed);
    doublerSliders[0].setValue(dblDelay, juce::dontSendNotification);
    // bypass
    auto dblBypassed = xml->getBoolAttribute("doublerBypassed",
        doublerBypassed.load(std::memory_order_relaxed));
    doublerBypassed.store(dblBypassed, std::memory_order_relaxed);
    doublerButton.setToggleState(dblBypassed, juce::dontSendNotification);
    //
    auto dblMix = static_cast<float>(
        xml->getDoubleAttribute("doublerMix",
            atomicMix.load(std::memory_order_relaxed)));
    atomicMix.store(dblMix, std::memory_order_relaxed);
    doublerSliders[3].setValue(dblMix, juce::dontSendNotification);
    */
