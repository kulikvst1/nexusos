...
{
    // получаем состояние клиппинга из мастер-метеров
    bool clipL = masterMeterL.isClipping();
    bool clipR = masterMeterR.isClipping();

    // если видимость LED не соответствует состоянию — обновляем
...
OutControlComponent::OutControlComponent()
{
   // setLookAndFeel(&customLF);
    ...
    for (int i = 0; i < 5; ++i)
    {
        ...
        for (int i = 0; i < 5; ++i)
        {
            ...
            if (i == 0) // Low-Cut
            {
                ...
                eqSliders[i].setColour(juce::Slider::backgroundColourId, juce::Colours::black);
            ...
            else if (i == 4) // High-Cut
            {
                ...
                eqSliders[i].setColour(juce::Slider::backgroundColourId, juce::Colours::darkgrey);
    ...
    gainSliderL.setTextBoxStyle(juce::Slider::TextBoxAbove, false, 80, 50);
    gainSliderL.setColour(juce::Slider::textBoxOutlineColourId, juce::Colours::transparentBlack);//цвет рамки
    gainSliderL.setColour(juce::Slider::trackColourId, juce::Colours::blue);
    gainSliderL.setColour(juce::Slider::backgroundColourId, juce::Colours::darkgrey);
    gainSliderL.setColour(juce::Slider::thumbColourId, juce::Colours::silver);
    gainSliderL.setLookAndFeel(nullptr);//custom slider (&customLF) or (nullptr);
    ...
    gainSliderR.setTextBoxStyle(juce::Slider::TextBoxAbove, false, 80, 50);
    gainSliderR.setColour(juce::Slider::textBoxOutlineColourId, juce::Colours::transparentBlack);//цвет рамки
    gainSliderR.setColour(juce::Slider::trackColourId, juce::Colours::blue);
    gainSliderR.setColour(juce::Slider::backgroundColourId, juce::Colours::darkgrey);
    gainSliderR.setColour(juce::Slider::thumbColourId, juce::Colours::silver);
    gainSliderR.setLookAndFeel(nullptr);//custom slider (&customLF) or (nullptr);
    ...
    meterModeButton.onClick = [this]()
        {
            ...
            meterModeButton.setButtonText(isPost ? "Post" : "Pre");
...
void OutControlComponent::resized()
{
    ...
    // 12) Располагаем CLIP-LED над кнопками Mute
    {
        auto placeLed = [&](juce::Label& led, const juce::TextButton& btn,
            int extraX, int extraY)
            {
                auto b = btn.getBounds();
                led.setBounds(b.getX() + extraX,
                    b.getY() - b.getHeight() - extraY,
                    b.getWidth(),
                    b.getHeight() / 2);
            };

        placeLed(clipLedL, muteButtonL, -65, 165);  
        placeLed(clipLedR, muteButtonR, 65, 165);  
    }
}
