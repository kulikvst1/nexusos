...
OutControlComponent::OutControlComponent()
{
    ...
    // === инициализация Doubler-слайдеров ===
        // === инициализация 4 Doubler-слайдеров ===
    for (auto& s : doublerSliders)
    {
        s.setSliderStyle(juce::Slider::RotaryHorizontalVerticalDrag);
    ...
    for (int i = 0; i < 4; ++i)
    {
        // оформление подписи ползунка
        addAndMakeVisible(doublerNameLabels[i]);
        doublerNameLabels[i].setText(doublerNames[i], juce::dontSendNotification);
        doublerNameLabels[i].setJustificationType(juce::Justification::centred);
        doublerNameLabels[i].setLookAndFeel(&customLF);
        doublerNameLabels[i].setColour(juce::Label::backgroundColourId, juce::Colours::white);
        doublerNameLabels[i].setColour(juce::Label::textColourId, juce::Colours::black);
        doublerNameLabels[i].setOpaque(true);
    }
    ...
    doublerLabel.setJustificationType(juce::Justification::centred);
    doublerLabel.setLookAndFeel(&customLF);
    doublerLabel.setColour(juce::Label::backgroundColourId, juce::Colours::white);
    doublerLabel.setColour(juce::Label::textColourId, juce::Colours::black);
    doublerLabel.setOpaque(true);
    // … теперь настраиваем Doubler-кнопку 
...
void OutControlComponent::resized()
{
    ...
    {
        constexpr int   pad = 4;   // внутренний отступ вокруг слайдера
        constexpr int   labelPad = 4;   // зазор между строкой имён и DABLER
        constexpr float nameW_pct = 0.6f;// ширина подписи = 60% от ширины клетки

        int lblH = gainLabelL.getHeight(); // высота DABLER-лейбла
        int smallH = lblH / 2;               // высота всех подписей

        // 1) объединяем ячейки (2,0) + (3,0) и делаем паддинг
        auto cell20 = grid.getSector(2, 0);
        auto cell30 = grid.getSector(3, 0);
        auto areaUnion = cell20.getUnion(cell30)
            .reduced(pad, pad);

        // 2) рисуем DABLER-лейбл штатной высоты внизу
        int ux = areaUnion.getX();
        int uw = areaUnion.getWidth();
        int uy = areaUnion.getBottom() - lblH;
        doublerLabel.setBounds(ux, uy, uw, lblH);
        doublerLabel.setFont({ lblH * 0.6f, juce::Font::bold });

        // 3) убираем снизу область под DABLER + небольшой зазор
        areaUnion.removeFromBottom(lblH + labelPad);

        // 4) дробим оставшуюся область пополам по высоте → topRow / bottomRow
        int halfH = areaUnion.getHeight() / 2;
        juce::Rectangle<int> topRow(
            areaUnion.getX(), areaUnion.getY(),
            areaUnion.getWidth(), halfH);

        juce::Rectangle<int> bottomRow(
            areaUnion.getX(),
            areaUnion.getY() + halfH,
            areaUnion.getWidth(),
            halfH);

        // 5) каждую строку делим пополам по ширине → получаем 4 клетки
        int halfW = topRow.getWidth() / 2;
        juce::Rectangle<int> cells[4] = {
            { topRow.getX(),             topRow.getY(),             halfW, topRow.getHeight() },
            { topRow.getX() + halfW,     topRow.getY(),             halfW, topRow.getHeight() },
            { bottomRow.getX(),          bottomRow.getY(),          halfW, bottomRow.getHeight() },
            { bottomRow.getX() + halfW,  bottomRow.getY(),          halfW, bottomRow.getHeight() }
        };

        // 6) во всех клетках: снизу — маленький лейбл (smallH) с уменьшенной шириной,
        //    над ним — сам слайдер
        for (int i = 0; i < 4; ++i)
        {
            // 6.1) вычисляем область подписи: центрируем nameW в ширине клетки
            int cellW = cells[i].getWidth();
            int nameW = int(cellW * nameW_pct);
            int nameX = cells[i].getX() + (cellW - nameW) / 2;
            int nameY = cells[i].getBottom() - smallH;

            doublerNameLabels[i].setBounds(
                nameX,
                nameY,
                nameW,
                smallH);

            // 6.2) сам ротационный слайдер — всё пространство сверху до nameY,
            //       с внутренним отступом pad
            int sliderH = nameY - cells[i].getY();
            ...
            doublerSliders[i].setBounds(sliderArea.reduced(pad));
        }
        // 7) Размещаем doublerButton над верхним левым слайдером (cells[0])
        auto eqB = bypassButton.getBounds();
        int  btnW = eqB.getWidth();
        int  btnH = eqB.getHeight();
        int  btnX = cells[0].getX() + (cells[0].getWidth() - btnW) / 2;
        int  btnY = cells[0].getY() - btnH - pad;

        doublerButton.setBounds(btnX, btnY, btnW, btnH);
    }
