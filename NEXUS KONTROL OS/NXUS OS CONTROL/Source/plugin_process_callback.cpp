    ...
    const juce::AudioIODeviceCallbackContext& /*ctx*/)
{
    ...
    if (inst == nullptr)
    {
        ...
        // ➡️ Вставляем InputControl прямо здесь
        if (inputControl != nullptr)
            inputControl->processBlock(stereoBuffer.getArrayOfWritePointers(), numSamples);
    ...
    for (int ch = numInputChannels; ch < totalCh; ++ch)
        inputBuffer.clear(ch, 0, numSamples);
    // ➡️ 2.5) Подаём сигнал в InputControl перед VST
    if (inputControl != nullptr)
        inputControl->processBlock(inputBuffer.getArrayOfWritePointers(), numSamples);
