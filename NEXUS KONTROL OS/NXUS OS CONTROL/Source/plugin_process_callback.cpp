...
//==============================================================================
//  setTuner – устанавливаем указатель и, если устройство уже запущено, сразу готовим буферы
//==============================================================================
void PluginProcessCallback::setTuner(TunerComponent* t) noexcept
{
    tuner = t;

    // Если аудиоустройство уже запущено, сразу вызываем prepare
    if (currentSampleRate > 0.0 && dryBuffer.getNumSamples() > 0)
        tuner->prepare(dryBuffer.getNumSamples(), currentSampleRate);
}
