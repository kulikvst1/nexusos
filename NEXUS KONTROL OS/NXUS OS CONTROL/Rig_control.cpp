    ...
    : deviceManager(adm)
{
    ...
    // 1. Контейнер для элементов интерфейса
    mainTab = std::make_unique<juce::Component>();
    addAndMakeVisible(mainTab.get());
    setSize(800, 600); // начальный размер
   

    // 2. Кнопки-пресеты (A, B, C)
    for (int i = 0; i < 3; ++i)
    {
        ...
        preset->setColour(juce::TextButton::buttonColourId, juce::Colour::fromRGB(200, 230, 255));
        ...
        sButtons[i]->onStateChange = [this, i, ccNumber]()
            {
                ...
                midiOut->sendMessageNow(
                    juce::MidiMessage::controllerEvent(1, ccNumber, value)
                );
...
volumeLabel.setFont(juce::Font(volumeSector.getHeight() * 0.17f, juce::Font::bold));
    ...
    // Looper и Tuner: показываем только если активны
    ...
    // ─── КНОПКИ SWITCH (S1–S4) ───
    if (sButtons[0]) sButtons[0]->setBounds(getSectorRect(10).reduced(1)); // S1
    if (sButtons[1]) sButtons[1]->setBounds(getSectorRect(19).reduced(1)); // S2
    if (sButtons[2]) sButtons[2]->setBounds(getSectorRect(18).reduced(1)); // S3
    if (sButtons[3]) sButtons[3]->setBounds(getSectorRect(27).reduced(1)); // S4

    // Обновляем визуальное состояние кнопок
    updateAllSButtons();

}
void Rig_control::setMidiOutput(const juce::String& identifier)
{
    midiOut.reset(); // закрыть старый порт

    if (identifier.isEmpty())
        return; // отключение выхода

    auto newOut = juce::MidiOutput::openDevice(identifier);
    if (newOut != nullptr)
    {
        midiOut = std::move(newOut);
...
void Rig_control::setBankEditor(BankEditor* editor)
{
    ...
    bankEditor->onBankChanged = [this]()
        {
            // Сброс Shift и Stomp через методы
            setShiftState(false);
            setStompState(false);

            // Возвращаем пресет‑кнопки в радиогруппу
            for (auto* btn : presetButtons)
...
void Rig_control::buttonClicked(juce::Button* button)
{
    ...
    if (stompMode && presetButtons.contains(static_cast<juce::TextButton*>(button)))
    {
        ...
        if (idx >= 0 && bankEditor)
        {
            ...
            // Используем точечный доступ, потому что shiftButton — не указатель
...
void Rig_control::updateAllSButtons()
{
    ...
    {
        if (looperActive) // используем looperActive как источник правды
    ...
    // S4
    // shiftButton — объект, проверка на nullptr не нужна и должна быть удалена
...
{
    if (!midiOut) // midiOut — unique_ptr, проверяем только его
        return;

    static std::atomic<bool> lastSentShift{ false }; // защита от дублей и потокобезопасность

    // shiftButton — объект, поэтому используем точечный доступ
    const bool shiftOn = shiftButton.getToggleState(); // <-- исправлена пропущенная ';'

    if (shiftOn != lastSentShift.load())
    {
        lastSentShift.store(shiftOn);
        const int value = shiftOn ? 127 : 0;
...
void Rig_control::setLooperState(bool on)
{
    ...
    // looperBtn — объект, используем точечный доступ
    looperBtn.setToggleState(on, juce::dontSendNotification);
    ...
    if (looperComponent)
        looperComponent->setVisible(embeddedLooperVisible && on);
...
void Rig_control::setEmbeddedLooperVisible(bool shouldShow)
{
    ...
    if (looperComponent)
        looperComponent->setVisible(embeddedLooperVisible);
}
...
void Rig_control::setTunerState(bool on)
{
    if (externalTuner)
    {
        ...
        if (on) externalTuner->toFront(false);
    }
    updateAllSButtons();
}
...
{
    const bool newState = !(externalTuner && externalTuner->isVisible());
...
{
    shiftButton.setToggleState(on, juce::dontSendNotification);

    if (stompMode)
    {
        manualShift = on;   // ← использовать текущее состояние Shift
        updateAllSButtons();
        sendShiftState();
        updateStompDisplays(); // сразу перерисовать кнопки
        return;
    }
    ...
    // если выключаем Shift и активный пресет > 2 → вернуть его в диапазон 0–2
    if (!manualShift && bankEditor)
    {
        int current = bankEditor->getActivePresetIndex();
        if (current >= 3)
        {
            int newIndex = current - 3;
            bankEditor->setActivePreset(newIndex);

            if (presetChangeCb)
                presetChangeCb(newIndex);
        }
    }
...
void Rig_control::toggleShift()
{
    // shiftButton — объект, проверка на nullptr не нужна
    shiftButton.toggle();
    setShiftState(shiftButton.getToggleState());
}
...
{
    // stompBtn — объект, используем точечный доступ
    stompBtn.setToggleState(on, juce::dontSendNotification);

    stompMode = on;

    if (stompMode)
    {
        // Запоминаем состояние Shift
        presetShiftState = shiftButton.getToggleState();

        // В стомп-режиме кнопки не радиогруппа
        for (auto* btn : presetButtons)
            if (btn) btn->setRadioGroupId(0, juce::dontSendNotification);

        // если Shift выключен и есть валидный bankEditor → вернуть активные CC в диапазон 0–2
        if (!presetShiftState && bankEditor)
        {
            const int bankIdx = bankEditor->getActiveBankIndex();
            const int presetIdx = bankEditor->getActivePresetIndex();

            auto& ccStates = bankEditor->getBank(bankIdx).ccPresetStates[presetIdx];
            const int maxCc = static_cast<int>(ccStates.size());

            for (int cc = 3; cc < 6; ++cc)
            {
                if (cc >= 0 && cc < maxCc && ccStates[cc])
                {
                    // переводим активный CC (3..5) в соответствующий 0..2
                    bankEditor->updateCCParameter(cc - 3, true);
                    bankEditor->updateCCParameter(cc, false);
                }
            }
        }
    ...
    {
        // Восстанавливаем состояние Shift — shiftButton объект, используем '.'
        shiftButton.setToggleState(presetShiftState, juce::dontSendNotification);

        for (auto* btn : presetButtons)
            if (btn) btn->setRadioGroupId(100, juce::dontSendNotification);
...
void Rig_control::toggleStomp()
{
    setStompState(!stompMode);
}


void Rig_control::tapTempoAction()
{
    if ((hostComponent == nullptr) || (hostComponent->getPluginInstance() == nullptr))
    {
        // дефолтный темп, уйдёт в MIDI через onBpmChanged
        hostComponent->updateBPM(120.0);
        return;
    }
    tapTempo.tap();
    double newBpm = tapTempo.getBpm();
