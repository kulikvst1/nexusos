    ...
    : deviceManager(adm)
{
    ...
    shiftButton = std::make_unique<FlagButton>();
    stompBtn = std::make_unique<FlagButton>();
    looperBtn = std::make_unique<FlagButton>();
...
void Rig_control::updateAllSButtons()
{
    ...
    {
        if (looperActive) // заменили looperBtn.getToggleState()
...
void Rig_control::setLooperState(bool on)
{
    ...
    if (enginePtr)
    {
        ...
        // 🔹 гарантированно отправляем режим наружу
    ...
    if (midiOut)
        midiOut->sendMessageNow(juce::MidiMessage::controllerEvent(1, 102, on ? 127 : 0));
...
void Rig_control::setStompState(bool on)
{
    ...
    if (stompMode)
    {
        ...
        // если Shift выключен и активный CC > 2 → вернуть его в диапазон 0–2
        if (!presetShiftState && bankEditor)
        {
            int bankIdx = bankEditor->getActiveBankIndex();
            int presetIdx = bankEditor->getActivePresetIndex();

            // проверяем активные CC
            for (int cc = 3; cc < 6; ++cc)
            {
                if (bankEditor->getBank(bankIdx).ccPresetStates[presetIdx][cc])
                {
    ...
    {
        // Восстанавливаем состояние Shift
        if (shiftButton != nullptr)
