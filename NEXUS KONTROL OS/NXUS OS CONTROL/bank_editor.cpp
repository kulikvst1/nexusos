...
void BankEditor::buttonClicked(juce::Button* b)
{
    ...
    else if (b == &storeButton)
    {
        auto doStore = [this]()
            {
                allowSave = true;
                storeToBank(); // сохраняем snapshot/файл
                allowSave = false;
                ...
                juce::File backupFile = saveDir.getChildFile(currentlyLoadedBankFile.getFileName());
                saveSettingsToFile(backupFile);

                // фиксируем точку правды для навигации
                currentlyLoadedBankFile = currentlyLoadedBankFile;

                DBG("[Store] saved to " << currentlyLoadedBankFile.getFullPathName()
                    << " and backup to " << backupFile.getFullPathName());
            };
...
void BankEditor::loadSettings()
{
    ...
    if (files.isEmpty())
    {
        // Первый запуск → создаём Default.xml
        juce::File defaultFile = sysDir.getChildFile("Default.xml");
        resetAllDefaults();
        saveSettingsToFile(defaultFile);
        currentlyLoadedBankFile = defaultFile;
        loadSettingsFromFile(currentlyLoadedBankFile);
        DBG("[Startup] created Default.xml");
    }
    else
    {
        // Загружаем существующий рабочий файл
        currentlyLoadedBankFile = files.getFirst();
        loadSettingsFromFile(currentlyLoadedBankFile);
        DBG("[Startup] loaded existing config: " << currentlyLoadedBankFile.getFullPathName());
    }
}
...
void BankEditor::loadSettingsFromFile(const juce::File& file)
{
    ...
    // --- Чтение глобальных и банковских данных (как у тебя есть) ---
    ...
    // ❌ Больше никаких удалений/копирований здесь
    currentlyLoadedBankFile = file;

    // Метка: имя плагина (если нужно имя файла — заменишь на getFileNameWithoutExtension())
    ...
    // Восстановление состояния плагина + UI
    juce::Timer::callAfterDelay(200, [this] {
...
void BankEditor::saveSettingsToFile(const juce::File& file)
{
    ...
    // ... формирование XML как у тебя ...

    bankSnapshot = banks[activeBankIndex];

    // 🔹 сохраняем в выбранный файл (например, BANK)
    file.replaceWithText(root.toString());

    // 🔹 теперь заменяем рабочий файл в системной папке
    auto sysDir = juce::File::getSpecialLocation(juce::File::userApplicationDataDirectory)
        .getChildFile("NEXUS_KONTROL_OS");
    sysDir.createDirectory();

    // удаляем старый рабочий файл
    auto oldFiles = sysDir.findChildFiles(juce::File::findFiles, false, "*.xml");
    for (auto& f : oldFiles) f.deleteFile();

    // копируем новый под тем же именем
    juce::File workingFile = sysDir.getChildFile(file.getFileName());
    file.copyFileTo(workingFile);

    currentlyLoadedBankFile = workingFile;
    pluginLabel.setText(workingFile.getFileNameWithoutExtension(), juce::dontSendNotification);

    DBG("[SaveSettings] replaced working file: " << workingFile.getFullPathName());
}


void BankEditor::resetAllDefaults()
{
    // 1. Сброс всех банков в память
    for (int i = 0; i < numBanks; ++i)
    {
        banks[i] = Bank();
    ...
    // 2. Выгружаем плагин, если он есть
    if (vstHost)
        vstHost->unloadPlugin();

    // 3. Ставим активный банк и пресет в начало
    setActiveBankIndex(0);
    setActivePreset(0);
    ...
    // 5. Фиксируем эталон, чтобы Store не мигала
    bankSnapshot = banks[activeBankIndex];

    // 6. Сохраняем текущее состояние в рабочий файл
    if (currentlyLoadedBankFile.existsAsFile())
        saveSettingsToFile(currentlyLoadedBankFile);
}
...
void BankEditor::loadBankFile(const juce::File& file)
{
    ...
    // Удаляем текущий рабочий файл (Только при явной загрузке нового!)
    if (currentlyLoadedBankFile.existsAsFile())
        currentlyLoadedBankFile.deleteFile();

    // Копируем новый в системную папку
    juce::File workingFile = sysDir.getChildFile(file.getFileName());
    file.copyFileTo(workingFile);
