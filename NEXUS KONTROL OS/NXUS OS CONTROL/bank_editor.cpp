        ...
        auto doStore = [this]()
            {
                ...
                if (currentlyLoadedBankFile.existsAsFile())
                {
                    ...
                    workingFile = sysDir.getChildFile(currentlyLoadedBankFile.getFileName());
                }
                else
                {
                    // первый раз → Default.xml
                ...
                juce::File backupFile = saveDir.getChildFile(workingFile.getFileName());
                if (backupFile.existsAsFile())
                    backupFile.deleteFile(); // удалить старый дубль

                saveSettingsToFile(backupFile);

                DBG("[Store] replaced working file: " << workingFile.getFullPathName()
                    << " and backup: " << backupFile.getFullPathName());
            };
...
void BankEditor::loadSettings()
{
    ...
    if (files.isEmpty())
    {
        // Пустая папка → создаём Default.xml и грузим его
        juce::File defaultFile = sysDir.getChildFile("Default.xml");
        resetAllDefaults();
        saveSettingsToFile(defaultFile);
        loadSettingsFromFile(defaultFile);
        DBG("[Startup] created Default.xml");
        return;
    }

    // Есть файл → грузим его (без удалений)
    juce::File workingFile = files.getFirst();
    loadSettingsFromFile(workingFile);
    DBG("[Startup] loaded: " << workingFile.getFullPathName());
}
...
void BankEditor::loadSettingsFromFile(const juce::File& file)
{
    ...
    // --- Чтение глобальных и банковских данных (оставь как в твоём коде) ---
    ...
    // Чисто фиксация пути рабочего файла; БЕЗ удалений/копирований
...
void BankEditor::loadBankFile(const juce::File& file)
{
    ...
    // 1. Удаляем старый рабочий файл
    if (currentlyLoadedBankFile.existsAsFile())
        currentlyLoadedBankFile.deleteFile();

    // 2. Копируем новый в системную папку
    juce::File workingFile = sysDir.getChildFile(file.getFileName());
    file.copyFileTo(workingFile);

    // 3. Загружаем его как рабочий
    loadSettingsFromFile(workingFile);
    currentlyLoadedBankFile = workingFile;
