        ...
        auto doStore = [this]()
            {
                ...
                // 🔹 сразу сохраняем в файл с именем библиотеки
                juce::File saveDir;
                if (GetDriveTypeW(L"D:\\") == DRIVE_FIXED)
                ...
                // всегда используем libraryName
                juce::String fileName = libraryName.trim();
                if (fileName.isEmpty())
                    fileName = "BANK"; // fallback

                if (!fileName.endsWithIgnoreCase(".xml"))
                    fileName += ".xml";

                juce::File targetFile = saveDir.getChildFile(fileName);

                // сохраняем прямо в этот файл
                snapshotCurrentBank();
                allowSave = true;
                storeToBank();
                allowSave = false;
                ...
                // фиксируем точку правды для навигации
                currentlyLoadedBankFile = targetFile;

                DBG("[Store] auto-saved to " << targetFile.getFullPathName());
            };
...
{
    libraryNameEditor.setText(libraryName, juce::dontSendNotification);
...
void BankEditor::loadSettingsFromFile(const juce::File& file)
{
    ...
    libraryName = xml->getStringAttribute("libraryName");
    ...
    juce::Timer::callAfterDelay(200, [this] {
        ...
        juce::Timer::callAfterDelay(200, [this] {
            ...
            // --- Синхронизация текущего файла для навигации ---
            auto files = scanNumericBankFiles();
            int prefix = getNumericPrefix(libraryName.trim());

            if (prefix >= 0)
            {
                for (auto& f : files)
                {
                    if (getNumericPrefix(f.getFileNameWithoutExtension()) == prefix)
                    {
                        currentlyLoadedBankFile = f;
                        break;
                    }
                }
            }
            ...
            {
                // если имя без номера → оставляем пусто,
                // навигация решит при первом клике (вперёд → первый, назад → последний)
                currentlyLoadedBankFile = juce::File();
            }
...
void BankEditor::saveSettingsToFile(const juce::File& file)
{
    ...
    root.setAttribute("libraryName", libraryName);
    // --- Глобальные данные о плагине ---
...
void BankEditor::storeToBank()
{
    ...
    // 🔹 Новое: глобальное имя библиотеки
    libraryName = libraryNameEditor.getText();
    ...
    // 🔹 Новое: сохраняем глобальное имя библиотеки
    root->setAttribute("libraryName", libraryName);
...
juce::XmlElement* BankEditor::serializeBank(const Bank& b, int index) const
{
    ...
    // 🔹 Новое поле: имя библиотеки
    bankEl->setAttribute("libraryName", libraryName); // <-- добавляем сюда
...
void BankEditor::deserializeBank(Bank& b, const juce::XmlElement& bankEl)
{
    b.bankName = bankEl.getStringAttribute("bankName");
    b.pluginName = bankEl.getStringAttribute("pluginName");
    libraryName = bankEl.getStringAttribute("libraryName");
...
// Компаратор для juce::Array::sort()
struct FileComparator
{
    ...
    {
        auto an = a.getFileNameWithoutExtension();
        auto bn = b.getFileNameWithoutExtension();

        int ap = editor->getNumericPrefix(an);
        int bp = editor->getNumericPrefix(bn);

        if (ap != bp)
            return (ap < bp) ? -1 : 1;

        // Вторичная сортировка: по имени (стабильно)
        return an.compare(bn);
    }
};

// Актуальный список файлов, начинающихся с цифры, отсортированный
juce::Array<juce::File> BankEditor::scanNumericBankFiles() const
{
    ...
    for (auto& f : files)
    {
        auto base = f.getFileNameWithoutExtension();
        if (getNumericPrefix(base) >= 0)
            result.add(f);
    }
...
// Где мы сейчас: точка правды текущего файла
juce::File BankEditor::getCurrentBankFile() const
{
    return currentlyLoadedBankFile;
}

// Загрузка выбранного файла (метка обновится внутри твоей логики)
void BankEditor::loadBankFile(const juce::File& file)
{
    if (!file.existsAsFile())
        return;

    loadSettingsFromFile(file);     // твоя существующая функция
    currentlyLoadedBankFile = file; // обновляем для навигации
}
void BankEditor::navigateBank(bool forward)
{
    ...
    auto currentBase = getCurrentBankFile().getFileNameWithoutExtension();
    int  currentPrefix = getNumericPrefix(currentBase);

    // Если текущий файл без числового префикса:
    // Вперёд -> минимальный, Назад -> максимальный
    if (currentPrefix < 0)
    {
    ...
    if (currentIndex == -1)
        return; // текущего файла нет в списке

    if (forward)
    {
        // Если не последний → следующий
        if (currentIndex < files.size() - 1)
            targetIndex = currentIndex + 1;
        else
            targetIndex = 0; // циклический переход на первый
    }
    else
    {
        // Если не первый → предыдущий
        if (currentIndex > 0)
            targetIndex = currentIndex - 1;
        else
            targetIndex = files.size() - 1; // циклический переход на последний
    }
