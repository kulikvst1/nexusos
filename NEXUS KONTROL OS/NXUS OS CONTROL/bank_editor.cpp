    ...
    : pluginManager(pm), vstHost(host)
{
    ...
    prevButton.setButtonText(juce::String::fromUTF8("🔽 Dn"));
    nextButton.setButtonText(juce::String::fromUTF8("🔼 Up"));
...
void BankEditor::resized()
{
    ...
    // Row 0: configSelectButton + libraryNameEditor + prev/next buttons
    {
        int bw = W / 8;
        int rowHeight = int(sH * shrinkFactor);
        int catalogWidth = 2 * bw;
        ...
        configSelectButton.setBounds(baseX + 0 * bw, catalogY, bw, rowHeight);
        prevButton.setBounds(catalogX - bw, catalogY, bw, rowHeight);
        nextButton.setBounds(catalogX + catalogWidth, catalogY, bw, rowHeight);
    }
    ...
    // Row 1: SELECT / PRESET1…6 / VST
    {
        ...
        for (int i = 0; i < 8; ++i)
        {
            juce::Button* b = (i == 0 ? &selectBankButton
                ...
                : &vstButton);
            b->setBounds(baseX + i * bw, baseY + 1 * sH, bw, rowHeight);
        }
    }
    ...
    // Row 2: Bank name editor, Preset editors, Plugin label
    {
        ...
        bankNameEditor.setBounds(baseX + 0 * fw, fy, fw, rowHeight);
        for (int i = 0; i < numPresets; ++i)
            presetEditors[i].setBounds(baseX + (i + 1) * fw, fy, fw, rowHeight);

        pluginLabel.setFont(juce::Font(sH * 0.45f, juce::Font::plain));
        pluginLabel.setJustificationType(juce::Justification::centredLeft);
        pluginLabel.setMinimumHorizontalScale(0.7f);
        pluginLabel.setBounds(baseX + 7 * fw, fy, fw, rowHeight);
    }

    // Row 3: selectedPresetLabel + bankIndexLabel (без уменьшения)
    {
        selectedPresetLabel.setBounds(baseX, baseY + 3 * sH, W, sH);
        bankIndexLabel.setFont(juce::Font(sH * 0.5f, juce::Font::bold));
        bankIndexLabel.setJustificationType(juce::Justification::centred);
        bankIndexLabel.setBounds(baseX, baseY + 3 * sH, sW, sH);
    }

    // Row 4–7: Learn/Set/CC name/CC toggle (уменьшенные)
    for (int i = 0; i < 10; ++i)
        learnButtons[i].setBounds(baseX + i * 2 * sW, baseY + 4 * sH, 2 * sW, int(sH * shrinkFactor));

    for (int i = 0; i < 10; ++i)
        setCCButtons[i].setBounds(baseX + i * 2 * sW, baseY + 5 * sH, 2 * sW, int(sH * shrinkFactor));

    for (int i = 0; i < 10; ++i)
        ccNameEditors[i].setBounds(baseX + i * 2 * sW, baseY + 6 * sH, 2 * sW, int(sH * shrinkFactor));

    for (int i = 0; i < 10; ++i)
        ccToggleButtons[i].setBounds(baseX + i * 2 * sW, baseY + 7 * sH, 2 * sW, int(sH * shrinkFactor));

    // Pedal block (без уменьшения)
    int startCol = 6;
    ...
    int y8 = baseY + 8 * sH + pedalYOffset;
    int y9 = baseY + 9 * sH + pedalYOffset;
    ...
    // Row 11: Default/Load/Store/Save/Cancel (без уменьшения)
...
void BankEditor::buttonClicked(juce::Button* b)
{
        ...
        auto doStore = [this]()
            {
                ...
                // 1) Определяем имя файла из метки библиотеки
                juce::String newName = libraryNameEditor.getText().trim();
                if (newName.isEmpty()) newName = "Default";

                // 2) Файл всегда создаётся/обновляется в BANK
                juce::File bankDir = (GetDriveTypeW(L"D:\\") == DRIVE_FIXED)
                    ? juce::File("D:\\NEXUS\\BANK")
                    : juce::File("C:\\NEXUS\\BANK");
                bankDir.createDirectory();
                ...
                // 3) Сохраняем в выбранный файл
                saveSettingsToFile(targetFile);
                currentlyLoadedBankFile = targetFile;

                // 4) Обновляем boot_config.xml
                writeBootConfig(targetFile);

                DBG("[Store] saved bank file: " << targetFile.getFullPathName());
            };
    ...
    else if (b == &cancelButton)
    {
        ...
        {
            juce::AlertWindow::showOkCancelBox(
                juce::AlertWindow::WarningIcon, juce::String::fromUTF8("⚠️ Cancel Changes"),
...
void BankEditor::loadSettings()
{
    ...
    // если путь битый → fallback на дефолтный банк
    if (!targetFile.existsAsFile())
    {
        juce::File defFile = juce::File("C:\\NEXUS\\BANK").getChildFile("Default.xml");
...
void BankEditor::resetAllDefaults()
{
    ...
    // 6) Создаём дефолтный файл в папке BANK
    juce::File bankDir = (GetDriveTypeW(L"D:\\") == DRIVE_FIXED)
        ? juce::File("D:\\NEXUS\\BANK")
        : juce::File("C:\\NEXUS\\BANK");
    bankDir.createDirectory();
    ...
    // Пересоздаём дефолтный конфиг в BANK
    saveSettingsToFile(defFile);
    ...
    // Обновляем название библиотеки в UI
    libraryNameEditor.setText("Default", juce::dontSendNotification);

    DBG("[Default] recreated Default.xml in BANK: " << defFile.getFullPathName());
}
    ...
    if (vstHost->getPluginInstance() == nullptr)
    {
        ...
        menu.showMenuAsync(
            ...
            [this, entries, indexMap](int choice)
            {
                ...
                updateVSTButtonLabel();
                bankSnapshot = banks[activeBankIndex]; // fix snapshot
            });
...
void BankEditor::checkForChanges()
{
    ...
    if (libraryNameEditor.getText() != loadedFileName)
    {
