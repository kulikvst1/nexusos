...
void BankEditor::storeToBank()
{
    ...
    if (vstHost != nullptr)
    {
        if (auto* inst = vstHost->getPluginInstance())
        {
            ...
            {
                // если JUCE не вернул путь — оставляем старое значение
                DBG("Warning: desc.fileOrIdentifier пустой, сохраняем старый pluginId");
                // b.pluginId уже содержит предыдущее значение
    ...
    // ❌ storeToBank теперь только обновляет banks[] и глобальные поля
    // Запись в файл делает saveSettingsToFile()
}
