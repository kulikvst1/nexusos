    ...
    : pluginManager(pm), vstHost(host)
{
    ...
    for (int i = 0; i < numPresets; ++i)
    {
        ...
        presetEditors[i].setFont(juce::Font(20.0f));
        presetEditors[i].setJustification(juce::Justification::centred);
        ...
        presetEditors[i].onTextChange = [this, i]()
            {
                ...
                // если изменён активный пресет → обновляем метку
                if (i == activePreset) {
                ...
                // проверяем изменения → кнопка Store мигает
                checkForChanges();
            };
    }
...
void BankEditor::editCCParameter(int ccIndex)
{
    ...
    new SetCCDialog(vstHost, initialMap, slotName,
        [this, ccIndex](CCMapping newMap, bool ok)
        {
            ...
            // 🔹 если имя пустое или "<none>", обновляем его из плагина
            if ((global.name.isEmpty() || global.name == "<none>") && newMap.paramIndex >= 0 && vstHost != nullptr) {
...
void BankEditor::loadSettingsFromFile(const juce::File& file)
{
    ...
    juce::Timer::callAfterDelay(200, [this] {
        ...
        bankSnapshot = banks[activeBankIndex]; // ✅ фиксируем эталон после применения
        isLoadingFromFile = false;
        updateUI();
        checkForChanges(); // ✅ проверка уже на актуальном состоянии
        });
    }
...
void BankEditor::resetAllDefaults()
{
    ...
    // 5) Работать только с системной папкой
    auto sysDir = getSysDir();
    sysDir.createDirectory();
    ...
    // 6) Обновляем название библиотеки
    libraryNameEditor.setText("Default", juce::dontSendNotification);

    // ✅ Фиксируем эталон только теперь
    bankSnapshot = banks[activeBankIndex];
    checkForChanges();
}
...
bool BankEditor::isModified() const
{
    const auto& current = banks[activeBankIndex];
    const auto& snapshot = bankSnapshot;

    // 1. Имя банка
    if (current.bankName != snapshot.bankName)
        return true;

    // 2. Имена пресетов
    for (int i = 0; i < numPresets; ++i)
        if (current.presetNames[i] != snapshot.presetNames[i])
            return true;

    // 3. Имена CC‑слотов
    for (int i = 0; i < numCCParams; ++i)
        if (current.globalCCMappings[i].name != snapshot.globalCCMappings[i].name)
            return true;

    // 4. Имя плагина
    if (current.pluginName != snapshot.pluginName)
        return true;

    return false; // никаких реальных изменений
}
...
void BankEditor::checkForChanges()
{
    ...
    // мигание кнопки Store только при реальных изменениях
    if (modified) {
