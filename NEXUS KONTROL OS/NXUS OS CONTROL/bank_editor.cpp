        ...
        auto doStore = [this]()
            {
                ...
                // имя рабочего файла: если уже есть — используем его имя, иначе Default.xml
                juce::File workingFile = currentlyLoadedBankFile.existsAsFile()
                    ? sysDir.getChildFile(currentlyLoadedBankFile.getFileName())
                    : sysDir.getChildFile("Default.xml");

                // удалить старый и записать новый
                ...
                // --- дубль в BANK ---
                juce::File saveDir = (GetDriveTypeW(L"D:\\") == DRIVE_FIXED)
...
void BankEditor::storeToBank()
{
    ...
    // ❌ Убираем сохранение в файл
    // storeToBank теперь только обновляет banks[activeBankIndex]
}
...
void BankEditor::loadBankFile(const juce::File& sourceFile)
{
    ...
    // рабочий файл всегда в системной папке
    juce::File workingFile = sysDir.getChildFile(sourceFile.getFileName());

    // удалить старый рабочий (если он есть)
    if (workingFile.existsAsFile())
        workingFile.deleteFile();

    // скопировать новый из BANK в системную папку
    if (!sourceFile.copyFileTo(workingFile))
        return;

    // загрузить его как рабочий
    loadSettingsFromFile(workingFile);
    currentlyLoadedBankFile = workingFile;
