...
void BankEditor::buttonClicked(juce::Button* b)
{
    ...
    else if (b == &storeButton)
    {
        auto doStore = [this]()
            {
                ...
                // 🔹 Обновляем loadedFileName, чтобы совпадало с libraryNameEditor
        ...
        // 🔹 Добавлена проверка имени библиотеки
...
void BankEditor::loadBankFile(const juce::File& sourceFile)
{
    ...
    // просто загружаем напрямую из BANK
    loadSettingsFromFile(sourceFile);
    currentlyLoadedBankFile = sourceFile;

    // и обновляем boot_config.xml ссылкой на этот файл
