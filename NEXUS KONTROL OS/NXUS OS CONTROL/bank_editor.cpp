...
{
    // читаем boot_config.xml
    juce::File targetFile = readBootConfig();

    if (!targetFile.existsAsFile())
    {
        // первый запуск → создаём дефолтный файл в BANK
        auto bankDir = juce::File("C:\\NEXUS\\BANK");
        bankDir.createDirectory();
        ...
        // записываем путь в boot_config.xml
        writeBootConfig(defFile);
        targetFile = defFile;

        DBG("[Startup] created default bank file: " << defFile.getFullPathName());
    }
    else
    {
        DBG("[Startup] using bank file from boot_config: " << targetFile.getFullPathName());
    }

    // грузим указанный файл
